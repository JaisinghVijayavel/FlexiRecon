...
DROP function IF EXISTS `fn_get_expressionformat_ds` $$
CREATE function `fn_get_expressionformat_ds`
(
  in_dataset_code varchar(32),
  in_set_dataset_field text,
  in_expression text,
  in_cumulative_flag boolean,
  in_cumulative_field varchar(128)
) returns text
me:begin
  ...
  if in_cumulative_flag = true then
    set v_expression = concat(in_cumulative_field,' := ',in_cumulative_field,' + ',v_expression);
  end if;

  if lower(mid(trim(in_set_dataset_field),1,3)) = 'col' and 1 = 2 then
