...
DROP FUNCTION IF EXISTS `fn_get_basefilterdsformat` $$
CREATE FUNCTION `fn_get_basefilterdsformat`
(
...
begin
  ...
  */
  ...
  set v_filter_field_type = fn_get_dsfieldtype(in_dataset_code,v_filter_org_field);
  ...
  if in_ident_value_flag <> 'Y' and lower(substr(in_ident_value,1,3)) = 'col' then
    set v_ident_field_type = fn_get_dsfieldtype(in_dataset_code,in_ident_value);
