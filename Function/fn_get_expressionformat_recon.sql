...
DROP function IF EXISTS `fn_get_expressionformat_recon` $$
CREATE function `fn_get_expressionformat_recon`
(
  in_recon_code varchar(32),
  in_set_recon_field text,
  in_expression text,
  in_cumulative_flag boolean,
  in_cumulative_variable text
) returns text
me:begin
  ...
  -- cumlative variable value calculation
  if in_cumulative_flag = true then
    set v_expression = concat(in_cumulative_variable,' := ',in_cumulative_variable,' + ',v_expression);
  end if;
