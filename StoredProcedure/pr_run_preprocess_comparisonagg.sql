...
DROP PROCEDURE IF EXISTS `pr_run_preprocess_comparisonagg` $$
CREATE PROCEDURE `pr_run_preprocess_comparisonagg`
(
...
me:BEGIN
  ...
  */
  ...
  declare v_preprocessagg_function text default '';
  declare v_preprocessagg_field text default '';
  declare v_preprocessagg_field_type text default '';

  declare v_preprocessagg_applied_on text default '';
  declare v_preprocessagg_criteria text default '';
  declare v_preprocessagg_value_flag text default '';
  declare v_preprocessagg_value text default '';

  declare v_preprocessagg_condition text default '';
  ...
  declare v_concurrent_ko_flag text default '';
	...
	declare v_preprocess_exp text default '';
  ...
  */
  ...
  drop temporary table if exists recon_tmp_t2preprocesspseudorows;
  drop temporary table if exists recon_tmp_t2preprocesstrangid;
  drop temporary table if exists recon_tmp_t2preprocesstranbrkpgid;
  drop temporary table if exists recon_tmp_t2preprocesstranwithbrkpgid;

  drop temporary table if exists recon_tmp_t2preprocessindex;
  drop temporary table if exists recon_tmp_t2preprocesssql;

  drop temporary table if exists recon_tmp_t2preprocesssourceagg;
  drop temporary table if exists recon_tmp_t2preprocesscomparisonagg;
  drop temporary table if exists recon_tmp_t2preprocesstranagg;

  CREATE TEMPORARY TABLE recon_tmp_t2preprocessindex(
  ...
  insert into recon_tmp_t2preprocessindex select 'recon_tmp_t2preprocesssource','idx_tran_date','Y';
  insert into recon_tmp_t2preprocessindex select 'recon_tmp_t2preprocesssource','idx_excp_value','Y';
  insert into recon_tmp_t2preprocessindex select 'recon_tmp_t2preprocesssource','idx_recon_code','Y';
  insert into recon_tmp_t2preprocessindex select 'recon_tmp_t2preprocesssource','idx_dataset_date','Y';

  insert into recon_tmp_t2preprocessindex select 'recon_tmp_t2preprocesscomparison','idx_tran_date','Y';
  insert into recon_tmp_t2preprocessindex select 'recon_tmp_t2preprocesscomparison','idx_excp_value','Y';
  insert into recon_tmp_t2preprocessindex select 'recon_tmp_t2preprocesscomparison','idx_recon_code','Y';
  insert into recon_tmp_t2preprocessindex select 'recon_tmp_t2preprocesscomparison','idx_dataset_date','Y';

  CREATE temporary TABLE recon_tmp_t2preprocesspseudorows(
    row int unsigned NOT NULL,
    PRIMARY KEY (row)
  ) ENGINE = MyISAM;

  insert into recon_tmp_t2preprocesspseudorows select 0 union select 1;

  CREATE temporary TABLE recon_tmp_t2preprocesstrangid(
    tran_gid int unsigned NOT NULL,
    PRIMARY KEY (tran_gid)
  ) ENGINE = MyISAM;

  CREATE temporary TABLE recon_tmp_t2preprocesstranbrkpgid(
  ...
  CREATE temporary TABLE recon_tmp_t2preprocesstranwithbrkpgid(
    tran_gid int unsigned not null,
    tranbrkp_gid int unsigned NOT NULL,
  ...
  CREATE temporary TABLE recon_tmp_t2preprocesssql(
  ...
  preprocess_block:begin
    ...
    declare preprocess_cursor cursor for
      ...
      and a.process_method = 'QCD_COMPARISONEXP_AGG'
      and a.hold_flag = 'N'
    ...
    preprocess_loop: loop
      ...
      set v_source_head_sql = concat('insert into recon_tmp_t2preprocesssource (',v_tran_fields,') ');
      ...
      set v_comparison_head_sql = concat('insert into recon_tmp_t2preprocesscomparison (',v_tran_fields,') ');
      ...
      set v_source_headbrkp_sql = concat('insert into recon_tmp_t2preprocesssource (',v_tranbrkp_fields,') ');
      ...
      set v_comparison_headbrkp_sql = concat('insert into recon_tmp_t2preprocesscomparison (',v_tranbrkp_fields,') ');
      ...
      if v_recontype_code <> 'N' then
        ...
        set v_comparison_headbrkp_sql = concat(v_comparison_headbrkp_sql,' and ko_gid = 0 ');
          ...
          preprocessfilter_block:begin
            ...
            and active_status = 'Y'
            and delete_flag = 'N'
            order by filter_applied_on,preprocessfilter_seqno;
          ...
          drop temporary table if exists recon_tmp_t2preprocesssource;
          drop temporary table if exists recon_tmp_t2preprocesscomparison;
          drop temporary table if exists recon_tmp_t2preprocesssourcedup;

          /*
          drop table if exists recon_tmp_t2preprocesssource;
          drop table if exists recon_tmp_t2preprocesscomparison;
          */

          create temporary table recon_tmp_t2preprocesssource select * from recon_trn_ttranwithbrkp where 1 = 2;
          alter table recon_tmp_t2preprocesssource ENGINE = MyISAM;
          alter table recon_tmp_t2preprocesssource add primary key(tran_gid,tranbrkp_gid);
          create index idx_excp_value on recon_tmp_t2preprocesssource(excp_value);
          create index idx_tran_date on recon_tmp_t2preprocesssource(tran_date);
          create index idx_recon_code on recon_tmp_t2preprocesssource(recon_code);
          create index idx_dataset_code on recon_tmp_t2preprocesssource(recon_code,dataset_code);

          create temporary table recon_tmp_t2preprocesscomparison select * from recon_trn_ttranwithbrkp where 1 = 2;
          alter table recon_tmp_t2preprocesscomparison ENGINE = MyISAM;
          alter table recon_tmp_t2preprocesscomparison add primary key(tran_gid,tranbrkp_gid);
          create index idx_excp_value on recon_tmp_t2preprocesscomparison(excp_value);
          create index idx_tran_date on recon_tmp_t2preprocesscomparison(tran_date);
          create index idx_recon_code on recon_tmp_t2preprocesscomparison(recon_code);
          create index idx_dataset_cdoe on recon_tmp_t2preprocesscomparison(recon_code,dataset_code);

          create temporary table recon_tmp_t2preprocesssourcedup select * from recon_trn_ttranwithbrkp where 1 = 2;
          alter table recon_tmp_t2preprocesssourcedup add primary key(tran_gid,tranbrkp_gid);
          create index idx_excp_value on recon_tmp_t2preprocesssourcedup(excp_value);
          create index idx_tran_date on recon_tmp_t2preprocesssourcedup(tran_date);
          create index idx_dataset_code on recon_tmp_t2preprocesssourcedup(recon_code,dataset_code);
          alter table recon_tmp_t2preprocesssourcedup ENGINE = MyISAM;
          ...
          /*
          drop table if exists recon_tmp_t2preprocesssourceagg;
          drop table if exists recon_tmp_t2preprocesscomparisonagg;
          drop table if exists recon_tmp_t2preprocesstranagg;
          */

          -- grouping temp table movement
          -- create agg temp tables
          create temporary table recon_tmp_t2preprocesssourceagg select * from recon_rpt_tpreprocessagg where 1 = 2;
          alter table recon_tmp_t2preprocesssourceagg ENGINE = MyISAM;
          alter table recon_tmp_t2preprocesssourceagg add primary key(preprocessagg_gid);
          alter table recon_tmp_t2preprocesssourceagg modify column preprocessagg_gid int unsigned AUTO_INCREMENT;

          create index idx_tran_value on recon_tmp_t2preprocesssourceagg(tran_value);
          create index idx_excp_value on recon_tmp_t2preprocesssourceagg(excp_value);
          create index idx_tran_date on recon_tmp_t2preprocesssourceagg(tran_date);

          create temporary table recon_tmp_t2preprocesscomparisonagg select * from recon_rpt_tpreprocessagg where 1 = 2;
          alter table recon_tmp_t2preprocesscomparisonagg ENGINE = MyISAM;
          alter table recon_tmp_t2preprocesscomparisonagg add primary key(preprocessagg_gid);
          alter table recon_tmp_t2preprocesscomparisonagg modify column preprocessagg_gid int unsigned AUTO_INCREMENT;

          create index idx_tran_value on recon_tmp_t2preprocesscomparisonagg(tran_value);
          create index idx_excp_value on recon_tmp_t2preprocesscomparisonagg(excp_value);
          create index idx_tran_date on recon_tmp_t2preprocesscomparisonagg(tran_date);

          create temporary table recon_tmp_t2preprocesstranagg select * from recon_rpt_tpreprocessagg where 1 = 2;
          alter table recon_tmp_t2preprocesstranagg ENGINE = MyISAM;
          alter table recon_tmp_t2preprocesstranagg add primary key(preprocessagg_gid);
          alter table recon_tmp_t2preprocesstranagg modify column preprocessagg_gid int unsigned AUTO_INCREMENT;

          create index idx_rec_count on recon_tmp_t2preprocesstranagg(rec_count);

          -- index table
          delete from recon_tmp_t2preprocessindex where sys_flag <> 'Y';
          truncate recon_tmp_t2preprocesssql;
          ...
          condition_block:begin
            ...
            and a.active_status = 'Y'
            and a.delete_flag = 'N'
            order by preprocesscondition_seqno;
            ...
            condition_loop: loop
              ...
              -- recon_tmp_t2preprocesssource
              if not exists(select index_name from recon_tmp_t2preprocessindex
                            WHERE table_name = 'recon_tmp_t2preprocesssource'
                ...
                insert into recon_tmp_t2preprocessindex(table_name,index_name) select 'recon_tmp_t2preprocesssource',v_index_name;
              end if;

              -- recon_tmp_t2preprocesssourceagg
              if mid(v_source_field,1,3) = 'col' then
                if not exists(select index_name from recon_tmp_t2preprocessindex
                  ...
                  insert into recon_tmp_t2preprocessindex(table_name,index_name) select 'recon_tmp_t2preprocesssourceagg',v_index_name;
                end if;

                -- recon_tmp_t2preprocesstranagg
                if not exists(select index_name from recon_tmp_t2preprocessindex
                            WHERE table_name = 'recon_tmp_t2preprocesstranagg'
                            and index_name = v_index_name) then
                  ...
                  insert into recon_tmp_t2preprocessindex(table_name,index_name) select 'recon_tmp_t2preprocesstranagg',v_index_name;
                end if;
              end if;

              -- recon_tmp_t2preprocesscomparison
              set v_index_name = concat('idx_',v_comparison_field);

              if not exists(select index_name from recon_tmp_t2preprocessindex
                            WHERE table_name = 'recon_tmp_t2preprocesscomparison'
                            and index_name = v_index_name) then
                ...
                insert into recon_tmp_t2preprocessindex(table_name,index_name) select 'recon_tmp_t2preprocesscomparison',v_index_name;
              end if;

              -- recon_tmp_t2preprocesscomparisonagg
              if mid(v_comparison_field,1,3) = 'col' then
                if not exists(select index_name from recon_tmp_t2preprocessindex
                  ...
                  insert into recon_tmp_t2preprocessindex(table_name,index_name) select 'recon_tmp_t2preprocesscomparisonagg',v_index_name;
                end if;
              ...
              if (instr(v_extraction_criteria,'$FIELD$') > 0)
                ...
                set v_sql = '';
                set v_sql = concat(v_sql,'update recon_tmp_t2preprocesssource set ');
                set v_sql = concat(v_sql,v_field,'=',v_field_format);

                insert into recon_tmp_t2preprocesssql(table_type,sql_query) values ('S',v_sql);
              ...
              if (instr(v_comparison_criteria,'$FIELD$') > 0)
                ...
                set v_sql = '';
                set v_sql = concat(v_sql,'update recon_tmp_t2preprocesscomparison set ');
                set v_sql = concat(v_sql,v_field,'=',v_field_format,' ');
                ...
                insert into recon_tmp_t2preprocesssql(table_type,sql_query) values ('C',v_sql);
          ...
          truncate recon_tmp_t2preprocesssource;
          truncate recon_tmp_t2preprocesscomparison;
          ...
          sql_block:begin
            ...
            select sql_query from recon_tmp_t2preprocesssql;
          ...
          -- preload pseudorows
          truncate recon_tmp_t2preprocesspseudorows;
          insert into recon_tmp_t2preprocesspseudorows select 0 union select 1;
          ...
          agg_source_block:begin
            ...
            from recon_mst_tpreprocessaggfieldhistory
            where preprocess_code = v_preprocess_code
            and recon_version = v_recon_version
            and preprocessaggfield_applied_on = 'S'
            and active_status = 'Y'
            and delete_flag = 'N'
            order by preprocessaggfield_seqno;
            ...
            agg_source_loop: loop
              fetch agg_source_cursor into v_recon_field,
                                           v_preprocessagg_function,
                                           v_preprocessagg_field,
                                           v_preprocessagg_field_type;
              if agg_source_done = 1 then leave agg_source_loop; end if;

              set v_txt = fn_get_fieldfunctionformat(v_recon_field,v_preprocessagg_field_type,v_preprocessagg_function);
              set v_txt = concat('cast(',v_txt,' as nchar)');

              set v_source_agg_field = concat(v_source_agg_field,',',v_preprocessagg_field);
            ...
            close agg_source_cursor;
					...
					set v_sql = concat('insert into recon_tmp_t2preprocesssourceagg (',substr(replace(v_source_groupby,'a.',''),2),v_source_agg_field,',');
					set v_sql = concat(v_sql,'rec_count,preprocessagg_json,src_comp_flag) ');
					...
					set v_sql = concat(v_sql,'from recon_tmp_t2preprocesssource ');
          ...
          agg_comparison_block:begin
            ...
            where preprocess_code = v_preprocess_code
            and recon_version = v_recon_version
            and preprocessaggfield_applied_on = 'C'
            and active_status = 'Y'
            and delete_flag = 'N'
            order by preprocessaggfield_seqno;
            ...
            agg_comparison_loop: loop
              ...
              set v_comparison_agg_field = concat(v_comparison_agg_field,',',v_preprocessagg_field);
            ...
            close agg_comparison_cursor;
					...
					set v_sql = concat('insert into recon_tmp_t2preprocesscomparisonagg (',substr(replace(v_comparison_groupby,'b.',''),2),v_comparison_agg_field,',');
					set v_sql = concat(v_sql,'rec_count,preprocessagg_json,src_comp_flag) ');
					...
					set v_sql = concat(v_sql,'from recon_tmp_t2preprocesscomparison ');
					set v_sql = concat(v_sql,'group by ',substr(replace(v_comparison_groupby,'b.',''),2));

          call pr_run_sql(v_sql,@result,@msg);

          set v_preprocessagg_condition = '';
          ...
          agg_condition_block:begin
            ...
            select
              a.preprocessagg_applied_on,
              a.preprocessagg_field,
              a.preprocessagg_criteria,
              a.preprocessagg_value_flag,
              a.preprocessagg_value,
              b.preprocessagg_field_type,
              a.open_parentheses_flag,
              a.close_parentheses_flag,
              a.join_condition
            from recon_mst_tpreprocessaggconditionhistory as a
            left join recon_mst_tpreprocessaggfieldhistory as b on a.preprocessagg_field = b.preprocessagg_field
              and a.preprocess_code = b.preprocess_code
              and a.recon_version = b.recon_version
              and a.preprocessagg_applied_on = b.preprocessaggfield_applied_on
            ...
            and a.active_status = 'Y'
            and a.delete_flag = 'N'
            order by a.preprocessaggcondition_seqno;
            ...
            agg_condition_loop: loop
              fetch agg_condition_cursor into v_preprocessagg_applied_on,
                                              v_preprocessagg_field,
                                              v_preprocessagg_criteria,
                                              v_preprocessagg_value_flag,
                                              v_preprocessagg_value,
                                              v_preprocessagg_field_type,
                                              v_open_parentheses_flag,
                                              v_close_parentheses_flag,
                                              v_join_condition;
              ...
              set v_preprocessagg_applied_on = ifnull(v_preprocessagg_applied_on,'');
              set v_preprocessagg_field = ifnull(v_preprocessagg_field,'');
              set v_preprocessagg_criteria = ifnull(v_preprocessagg_criteria,'');
              set v_preprocessagg_value_flag = ifnull(v_preprocessagg_value_flag,'');
              set v_preprocessagg_value = ifnull(v_preprocessagg_value,'');
              ...
              set v_txt = fn_get_preprocessaggcondition(v_preprocessagg_applied_on,
                                                   v_preprocessagg_field,
                                                   v_preprocessagg_field_type,
                                                   v_preprocessagg_criteria,
                                                   v_preprocessagg_value_flag,
              ...
              if v_preprocessagg_condition = '' then
                set v_preprocessagg_condition = ' and ';
              end if;

              set v_preprocessagg_condition = concat(v_preprocessagg_condition,
                                                v_open_parentheses_flag,
                                                v_txt,
          ...
          -- inner join
          set v_sql = concat('insert into recon_tmp_t2preprocesstranagg(rec_count,preprocessagg_json) ');
          set v_sql = concat(v_sql,'select (a.rec_count+b.rec_count),JSON_MERGE_PRESERVE(a.preprocessagg_json,b.preprocessagg_json) ');
          set v_sql = concat(v_sql,'from recon_tmp_t2preprocesssourceagg as a ');
          set v_sql = concat(v_sql,'inner join recon_tmp_t2preprocesscomparisonagg as b on 1 = 1 ',v_preprocess_condition);
          set v_sql = concat(v_sql,'where 1 = 1 ',v_preprocessagg_condition);

          call pr_run_sql(v_sql,@result,@msg);

          -- left join
          set v_sql = concat('insert into recon_tmp_t2preprocesstranagg(rec_count,preprocessagg_json) ');
          set v_sql = concat(v_sql,'select a.rec_count,a.preprocessagg_json ');
          set v_sql = concat(v_sql,'from recon_tmp_t2preprocesssourceagg as a ');
          set v_sql = concat(v_sql,'left join recon_tmp_t2preprocesscomparisonagg as b on 1 = 1 ',v_preprocess_condition);
          set v_sql = concat(v_sql,'where 1 = 1 ',v_preprocessagg_condition);
          set v_sql = concat(v_sql,'and b.preprocessagg_gid is null ');

          call pr_run_sql(v_sql,@result,@msg);

          -- right join
          set v_sql = concat('insert into recon_tmp_t2preprocesstranagg(rec_count,preprocessagg_json) ');
          set v_sql = concat(v_sql,'select b.rec_count,b.preprocessagg_json ');
          set v_sql = concat(v_sql,'from recon_tmp_t2preprocesssourceagg as a ');
          set v_sql = concat(v_sql,'right join recon_tmp_t2preprocesscomparisonagg as b on 1 = 1 ',v_preprocess_condition);
          set v_sql = concat(v_sql,'where 1 = 1 ',v_preprocessagg_condition);
          set v_sql = concat(v_sql,'and a.preprocessagg_gid is null ');

          call pr_run_sql(v_sql,@result,@msg);

          -- insert in tranbrkp_gid table
          select max(rec_count) into v_count from recon_tmp_t2preprocesstranagg;
          set v_count = ifnull(v_count,0);

          truncate recon_tmp_t2preprocesspseudorows;

          if v_count >= 2 then
            insert into recon_tmp_t2preprocesspseudorows select row from pseudo_rows1 where row <= v_count;
          else
            insert into recon_tmp_t2preprocesspseudorows select 0 union select 1;
          end if;

          truncate recon_tmp_t2preprocesstranwithbrkpgid;

					insert ignore into recon_tmp_t2preprocesstranwithbrkpgid (tran_gid,tranbrkp_gid)
						select
							JSON_UNQUOTE(JSON_EXTRACT(recon_tmp_t2preprocesstranagg.preprocessagg_json, CONCAT('$[', recon_tmp_t2preprocesspseudorows.row, '].tran_gid'))) AS tran_gid,
							JSON_UNQUOTE(JSON_EXTRACT(recon_tmp_t2preprocesstranagg.preprocessagg_json, CONCAT('$[', recon_tmp_t2preprocesspseudorows.row, '].tranbrkp_gid'))) AS tranbrkp_gid
						FROM recon_tmp_t2preprocesstranagg
						JOIN recon_tmp_t2preprocesspseudorows
						where rec_count > 0
						HAVING tran_gid IS NOT NULL;

					-- reconexp block
					set v_preprocess_exp = '';
					...
					reconexp_block:begin
						...
						reconexp_loop: loop
							fetch reconexp_cursor into v_set_recon_field,v_expression;
							if reconexp_done = 1 then leave reconexp_loop; end if;

							set v_expression = fn_get_expressionformat(in_recon_code,v_set_recon_field,v_expression,false);
							
							if v_preprocess_exp = '' then
								set v_preprocess_exp = concat(v_set_recon_field,' = ',v_expression,' ');
							else
								set v_preprocess_exp = concat(v_preprocess_exp,',',v_set_recon_field,' = ',v_expression,' ');
							end if;
          ...
          -- update preprocess
					if v_preprocess_exp <> '' then
						...
						set v_sql = concat("
							...
							v_preprocess_exp,	
							"where a.tran_gid in (select b.tran_gid from recon_tmp_t2preprocesstranwithbrkpgid as b
						...
						set v_sql = concat("
							...
							v_preprocess_exp,	
							"where (a.tran_gid,a.tranbrkp_gid) in (select b.tran_gid,b.tranbrkp_gid from recon_tmp_t2preprocesstranwithbrkpgid as b
          ...
          delete from recon_tmp_t2preprocessindex
          where table_name in ('recon_tmp_t2preprocesssourceagg','recon_tmp_t2preprocesscomparisonagg','recon_tmp_t2preprocesstranagg');

          truncate recon_tmp_t2preprocesssource;
          truncate recon_tmp_t2preprocesscomparison;
          truncate recon_tmp_t2preprocesssourcedup;
          truncate recon_tmp_t2preprocesstrangid;
          truncate recon_tmp_t2preprocesstranbrkpgid;

          truncate recon_tmp_t2preprocesssourceagg;
          truncate recon_tmp_t2preprocesscomparisonagg;
          truncate recon_tmp_t2preprocesstranagg;
