...
CREATE PROCEDURE `pr_recon_mst_trulesetup`(
    ...
    in in_rule_automatch_partial char(1),
    in in_threshold_flag  char(1),
...
me:BEGIN
  ...
  declare v_manual_match_flag text default '';
	...
	if in_applyrule_on = 'S' and in_comparison_dataset_code = '' and in_parent_dataset_code <> '' then
	...
	if(in_action = 'INSERT' or in_action = 'UPDATE') then
        ...
        if in_source_acc_mode = '' or in_source_acc_mode is null then
			set err_msg := concat(err_msg,'Source Account Mode cannot be empty,');
			set err_flag := true;
		end if;
        if in_comparison_acc_mode = '' or in_comparison_acc_mode is null then
			set err_msg := concat(err_msg,'Account Mode cannot be empty,');
			set err_flag := true;
		end if;
		 if in_rule_order = 0 or in_rule_order is null then
			set err_msg := concat(err_msg,'Rule Order cannot be zero,');
			set err_flag := true;
		end if;
		...
		if in_rule_automatch_partial ='Y' THEN
        ...
        if in_group_flag is null or in_group_flag = ''  then
		...
		if in_probableflag <> 'Y' and in_probableflag <> 'N' or in_probableflag is null then
			set err_msg := concat(err_msg,'Invalid probableflag value,');
			set err_flag := true;
		elseif in_probableflag = 'Y' then
			set v_system_match_flag = 'N';
      set v_manual_match_flag = 'Y';
		elseif in_probableflag = 'N' then
			set v_system_match_flag = 'Y';
      set v_manual_match_flag = 'N';
		end if;
	end if;

  
	if in_action ='INSERT' then
		if exists(select rule_gid from recon_mst_trule where rule_name = in_rule_name and recon_code = in_recon_code and active_status ='Y' and delete_flag = 'N') then
			set err_msg := concat(err_msg,'Duplicate record,');
			set err_flag := true;
		end if;
    elseif in_action ='UPDATE' then
		if exists(select rule_gid from recon_mst_trule where rule_name = in_rule_name and recon_code = in_recon_code and rule_gid <> in_rule_gid
	...
	if in_action ='INSERT' then
		if exists(select rule_gid from recon_mst_trule where recon_code = in_recon_code and rule_order = in_rule_order 	and active_status ='Y' and delete_flag = 'N') then
			set err_msg := concat(err_msg,'Duplicate rule order no.,');
			set err_flag := true;
		end if;
    elseif in_action ='UPDATE' then
		if exists(select rule_gid from recon_mst_trule where recon_code = in_recon_code and rule_order = in_rule_order and rule_gid <> in_rule_gid and delete_flag = 'N' and active_status='Y') then
			set err_msg := concat(err_msg,'Duplicate rule order no.,');
			set err_flag := true;
		end if;
	end if;
	...
	if in_action = "UPDATE"  or in_action = "DELETE" then
		if not exists(select rule_gid from recon_mst_trule where rule_gid = in_rule_gid and delete_flag = 'N') then
	...
	if in_action = 'UPDATE' then
		select count(*) into v_result from recon_mst_trulecondition where rule_code = in_rule_code and delete_flag = 'N';
  ...
  start transaction;
	...
	elseif(in_action = 'UPDATE') then
		update recon_mst_trule set
            ...
            manual_match_flag = v_manual_match_flag,
            probable_match_flag		= in_probableflag,
            threshold_flag 			= in_threshold_flag,
            rule_automatch_partial	= in_rule_automatch_partial,         
		...
		where rule_gid = in_rule_gid and delete_flag = 'N';
