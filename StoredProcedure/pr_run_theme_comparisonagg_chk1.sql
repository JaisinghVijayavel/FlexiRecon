...
DROP PROCEDURE IF EXISTS `pr_run_theme_comparisonagg_chk1` $$
CREATE PROCEDURE `pr_run_theme_comparisonagg_chk1`
(
...
me:BEGIN
  ...
  */
  ...
  drop temporary table if exists recon_tmp_t1themetrangid;
  drop temporary table if exists recon_tmp_t1themetranbrkpgid;
  drop temporary table if exists recon_tmp_t1themetranwithbrkpgid;

  drop temporary table if exists recon_tmp_t1themeindex;
  drop temporary table if exists recon_tmp_t1themesql;

  drop temporary table if exists recon_tmp_t1themesourceagg;
  drop temporary table if exists recon_tmp_t1themecomparisonagg;
  drop temporary table if exists recon_tmp_t1themetranagg;

  drop temporary table if exists recon_tmp_t1themematch;
  drop temporary table if exists recon_tmp_t1themematchdtl;
  drop temporary table if exists recon_tmp_t1thememanymatch;
  drop temporary table if exists recon_tmp_t1themematchdtlgid;
  drop temporary table if exists recon_tmp_t1themematchdup;
  drop temporary table if exists recon_tmp_t1themematchparentgid;

  drop table if exists recon_tmp_t1themematch;
  drop table if exists recon_tmp_t1themematchdtl;
  drop table if exists recon_tmp_t1thememanymatch;

  CREATE /*temporary*/ TABLE recon_tmp_t1themematch(
  ...
  create /*temporary*/ table recon_tmp_t1themematchdtl(
  ...
  CREATE /*temporary*/ TABLE recon_tmp_t1thememanymatch(
    tran_gid int unsigned NOT NULL,
    tranbrkp_gid int unsigned not null default 0,
  ...
  create temporary table recon_tmp_t1themematchdtlgid(
    matchdtl_gid int unsigned NOT NULL,
    PRIMARY KEY (matchdtl_gid)
  ) ENGINE = MyISAM;

  create temporary table recon_tmp_t1themematchdup(
  ...
  create temporary table recon_tmp_t1themematchparentgid(
  ...
  CREATE TEMPORARY TABLE recon_tmp_t1themeindex(
  ...
  insert into recon_tmp_t1themeindex select 'recon_tmp_t1themesource','idx_tran_date','Y';
  insert into recon_tmp_t1themeindex select 'recon_tmp_t1themesource','idx_excp_value','Y';
  insert into recon_tmp_t1themeindex select 'recon_tmp_t1themesource','idx_recon_code','Y';
  insert into recon_tmp_t1themeindex select 'recon_tmp_t1themesource','idx_dataset_date','Y';

  insert into recon_tmp_t1themeindex select 'recon_tmp_t1themecomparison','idx_tran_date','Y';
  insert into recon_tmp_t1themeindex select 'recon_tmp_t1themecomparison','idx_excp_value','Y';
  insert into recon_tmp_t1themeindex select 'recon_tmp_t1themecomparison','idx_recon_code','Y';
  insert into recon_tmp_t1themeindex select 'recon_tmp_t1themecomparison','idx_dataset_date','Y';

  drop temporary table if exists recon_tmp_t1themepseudorows;

  CREATE temporary TABLE recon_tmp_t1themepseudorows(
    row int unsigned NOT NULL,
    PRIMARY KEY (row)
  ) ENGINE = MyISAM;

  insert into recon_tmp_t1themepseudorows select 0 union select 1;

  CREATE temporary TABLE recon_tmp_t1themetrangid(
    tran_gid int unsigned NOT NULL,
    match_gid int not null default 0,
    PRIMARY KEY (tran_gid)
  ) ENGINE = MyISAM;

  CREATE temporary TABLE recon_tmp_t1themetranbrkpgid(
  ...
  CREATE temporary TABLE recon_tmp_t1themetranwithbrkpgid(
    tran_gid int unsigned not null,
    tranbrkp_gid int unsigned NOT NULL,
  ...
  CREATE temporary TABLE recon_tmp_t1themesql(
  ...
  theme_block:begin
    ...
    theme_loop: loop
      ...
      set v_source_head_sql = concat('insert into recon_tmp_t1themesource (',v_tran_fields,') ');
      ...
      set v_comparison_head_sql = concat('insert into recon_tmp_t1themecomparison (',v_tran_fields,') ');
      ...
      set v_source_headbrkp_sql = concat('insert into recon_tmp_t1themesource (',v_tranbrkp_fields,') ');
      ...
      set v_comparison_headbrkp_sql = concat('insert into recon_tmp_t1themecomparison (',v_tranbrkp_fields,') ');
      ...
      if v_recontype_code <> 'N' then
        ...
        set v_comparison_headbrkp_sql = concat(v_comparison_headbrkp_sql,' and ko_gid = 0 ');
          ...
          drop temporary table if exists recon_tmp_t1themesource;
          drop temporary table if exists recon_tmp_t1themecomparison;
          drop temporary table if exists recon_tmp_t1themesourcedup;

          drop table if exists recon_tmp_t1themesource;
          drop table if exists recon_tmp_t1themecomparison;

          create /*temporary*/ table recon_tmp_t1themesource select * from recon_trn_ttranwithbrkp where 1 = 2;
          alter table recon_tmp_t1themesource ENGINE = MyISAM;
          alter table recon_tmp_t1themesource add primary key(tran_gid,tranbrkp_gid);
          create index idx_excp_value on recon_tmp_t1themesource(excp_value);
          create index idx_tran_date on recon_tmp_t1themesource(tran_date);
          create index idx_recon_code on recon_tmp_t1themesource(recon_code);
          create index idx_dataset_code on recon_tmp_t1themesource(recon_code,dataset_code);

          create /*temporary*/ table recon_tmp_t1themecomparison select * from recon_trn_ttranwithbrkp where 1 = 2;
          alter table recon_tmp_t1themecomparison ENGINE = MyISAM;
          alter table recon_tmp_t1themecomparison add primary key(tran_gid,tranbrkp_gid);
          create index idx_excp_value on recon_tmp_t1themecomparison(excp_value);
          create index idx_tran_date on recon_tmp_t1themecomparison(tran_date);
          create index idx_recon_code on recon_tmp_t1themecomparison(recon_code);
          create index idx_dataset_cdoe on recon_tmp_t1themecomparison(recon_code,dataset_code);

          create temporary table recon_tmp_t1themesourcedup select * from recon_trn_ttranwithbrkp where 1 = 2;
          alter table recon_tmp_t1themesourcedup add primary key(tran_gid,tranbrkp_gid);
          create index idx_excp_value on recon_tmp_t1themesourcedup(excp_value);
          create index idx_tran_date on recon_tmp_t1themesourcedup(tran_date);
          create index idx_dataset_code on recon_tmp_t1themesourcedup(recon_code,dataset_code);
          alter table recon_tmp_t1themesourcedup ENGINE = MyISAM;

          drop temporary table if exists recon_tmp_t1themesourceagg;
          drop temporary table if exists recon_tmp_t1themecomparisonagg;
          drop temporary table if exists recon_tmp_t1themetranagg;

          drop table if exists recon_tmp_t1themesourceagg;
          drop table if exists recon_tmp_t1themecomparisonagg;
          drop table if exists recon_tmp_t1themetranagg;

          -- grouping temp table movement
          -- create agg temp tables
          create /*temporary*/ table recon_tmp_t1themesourceagg select * from recon_rpt_tthemeagg where 1 = 2;
          alter table recon_tmp_t1themesourceagg ENGINE = MyISAM;
          alter table recon_tmp_t1themesourceagg add primary key(themeagg_gid);
          alter table recon_tmp_t1themesourceagg modify column themeagg_gid int unsigned AUTO_INCREMENT;
          alter table recon_tmp_t1themesourceagg add match_gid int not null default 0;

          create index idx_tran_value on recon_tmp_t1themesourceagg(tran_value);
          create index idx_excp_value on recon_tmp_t1themesourceagg(excp_value);
          create index idx_tran_date on recon_tmp_t1themesourceagg(tran_date);
          create index idx_match_gid on recon_tmp_t1themesourceagg(match_gid);

          create /*temporary*/ table recon_tmp_t1themecomparisonagg select * from recon_rpt_tthemeagg where 1 = 2;
          alter table recon_tmp_t1themecomparisonagg ENGINE = MyISAM;
          alter table recon_tmp_t1themecomparisonagg add primary key(themeagg_gid);
          alter table recon_tmp_t1themecomparisonagg modify column themeagg_gid int unsigned AUTO_INCREMENT;
          alter table recon_tmp_t1themecomparisonagg add match_gid int not null default 0;

          create index idx_tran_value on recon_tmp_t1themecomparisonagg(tran_value);
          create index idx_excp_value on recon_tmp_t1themecomparisonagg(excp_value);
          create index idx_tran_date on recon_tmp_t1themecomparisonagg(tran_date);
          create index idx_match_gid on recon_tmp_t1themecomparisonagg(match_gid);

          create /*temporary*/ table recon_tmp_t1themetranagg select * from recon_rpt_tthemeagg where 1 = 2;
          alter table recon_tmp_t1themetranagg ENGINE = MyISAM;
          alter table recon_tmp_t1themetranagg add primary key(themeagg_gid);
          alter table recon_tmp_t1themetranagg modify column themeagg_gid int unsigned AUTO_INCREMENT;

          create index idx_rec_count on recon_tmp_t1themetranagg(rec_count);

          -- index table
          delete from recon_tmp_t1themeindex where sys_flag <> 'Y';
          truncate recon_tmp_t1themesql;
          ...
          condition_block:begin
            ...
            condition_loop: loop
              ...
              -- recon_tmp_t1themesource
              if not exists(select index_name from recon_tmp_t1themeindex
                            WHERE table_name = 'recon_tmp_t1themesource'
                            and index_name = v_index_name) then

                if substr(v_source_field,1,3) = 'col' then
                  set v_index_sql = concat('create index idx_',v_source_field,' on recon_tmp_t1themesource(',v_source_field,'(255))');
                else
                  set v_index_sql = concat('create index idx_',v_source_field,' on recon_tmp_t1themesource(',v_source_field,')');
                end if;

                call pr_run_sql(v_index_sql,@msg,@result);

                insert into recon_tmp_t1themeindex(table_name,index_name) select 'recon_tmp_t1themesource',v_index_name;
              end if;

              -- recon_tmp_t1themesourceagg
              if mid(v_source_field,1,3) = 'col' then
                if not exists(select index_name from recon_tmp_t1themeindex
                            WHERE table_name = 'recon_tmp_t1themesourceagg'
                            and index_name = v_index_name) then

                  set v_index_sql = concat('create index idx_',v_source_field,' on recon_tmp_t1themesourceagg(',v_source_field,'(255))');

                  call pr_run_sql(v_index_sql,@msg,@result);

                  insert into recon_tmp_t1themeindex(table_name,index_name) select 'recon_tmp_t1themesourceagg',v_index_name;
                end if;

                -- recon_tmp_t1themetranagg
                if not exists(select index_name from recon_tmp_t1themeindex
                            WHERE table_name = 'recon_tmp_t1themetranagg'
                            and index_name = v_index_name) then

                  if substr(v_source_field,1,3) = 'col' then
                    set v_index_sql = concat('create index idx_',v_source_field,' on recon_tmp_t1themetranagg(',v_source_field,'(255))');
                  else
                    set v_index_sql = concat('create index idx_',v_source_field,' on recon_tmp_t1themetranagg(',v_source_field,')');
                  end if;

                  call pr_run_sql(v_index_sql,@msg,@result);

                  insert into recon_tmp_t1themeindex(table_name,index_name) select 'recon_tmp_t1themetranagg',v_index_name;
                end if;
              end if;

              -- recon_tmp_t1themecomparison
              set v_index_name = concat('idx_',v_comparison_field);

              if not exists(select index_name from recon_tmp_t1themeindex
                            WHERE table_name = 'recon_tmp_t1themecomparison'
                            and index_name = v_index_name) then

                if substr(v_comparison_field,1,3) = 'col' then
                  set v_index_sql = concat('create index idx_',v_comparison_field,' on recon_tmp_t1themecomparison(',v_comparison_field,'(255))');
                else
                  set v_index_sql = concat('create index idx_',v_comparison_field,' on recon_tmp_t1themecomparison(',v_comparison_field,')');
                end if;

                call pr_run_sql(v_index_sql,@msg,@result);

                insert into recon_tmp_t1themeindex(table_name,index_name) select 'recon_tmp_t1themecomparison',v_index_name;
              end if;

              -- recon_tmp_t1themecomparisonagg
              if mid(v_comparison_field,1,3) = 'col' then
                if not exists(select index_name from recon_tmp_t1themeindex
                            WHERE table_name = 'recon_tmp_t1themecomparisonagg'
                            and index_name = v_index_name) then

                  set v_index_sql = concat('create index idx_',v_comparison_field,' on recon_tmp_t1themecomparisonagg(',v_comparison_field,'(255))');

                  call pr_run_sql(v_index_sql,@msg,@result);

                  insert into recon_tmp_t1themeindex(table_name,index_name) select 'recon_tmp_t1themecomparisonagg',v_index_name;
                end if;
                ...
                if not exists(select index_name from recon_tmp_t1themeindex
                  ...
                  if substr(v_comparison_field,1,3) = 'col' then
                    set v_index_sql = concat('create index idx_',v_comparison_field,' on recon_tmp_t1themetranagg(',v_comparison_field,'(255))');
                  else
                    set v_index_sql = concat('create index idx_',v_comparison_field,' on recon_tmp_t1themetranagg(',v_comparison_field,')');
                  end if;
              ...
              if (instr(v_extraction_criteria,'$FIELD$') > 0)
                ...
                set v_sql = '';
                set v_sql = concat(v_sql,'update recon_tmp_t1themesource set ');
                set v_sql = concat(v_sql,v_field,'=',v_field_format);

                insert into recon_tmp_t1themesql(table_type,sql_query) values ('S',v_sql);
              ...
              if (instr(v_comparison_criteria,'$FIELD$') > 0)
                ...
                set v_sql = '';
                set v_sql = concat(v_sql,'update recon_tmp_t1themecomparison set ');
                set v_sql = concat(v_sql,v_field,'=',v_field_format,' ');
                ...
                insert into recon_tmp_t1themesql(table_type,sql_query) values ('C',v_sql);
          ...
          truncate recon_tmp_t1themesource;
          truncate recon_tmp_t1themecomparison;
          ...
          sql_block:begin
            ...
            select sql_query from recon_tmp_t1themesql;
          ...
          -- preload pseudorows
          truncate recon_tmp_t1themepseudorows;
          insert into recon_tmp_t1themepseudorows select 0 union select 1;
          ...
          alter table recon_tmp_t1themesource add match_flag char(1) not null default 'N';
          alter table recon_tmp_t1themesource add match_gid int not null default 0;

          alter table recon_tmp_t1themecomparison add match_flag char(1) not null default 'N';
          alter table recon_tmp_t1themecomparison add match_gid int not null default 0;
          ...
          -- start matching grouping part
					...
					if v_manytomany_match_flag = 'Y' then
						set v_match_sql = 'insert ignore into recon_tmp_t1thememanymatch (tran_gid,tranbrkp_gid,matched_count,';
						...
						set v_match_sql = concat(v_match_sql,'from recon_tmp_t1themesource as a ');
						set v_match_sql = concat(v_match_sql,'inner join recon_tmp_t1themecomparison as b ');
            ...
            select v_match_sql;
						...
						set v_match_sql = 'insert into recon_tmp_t1themematch (group_flag,tran_gid,tranbrkp_gid,matched_count,matched_value,matched_json) ';
						...
						set v_match_sql = concat(v_match_sql,'from recon_tmp_t1thememanymatch ');
            ...
            select v_match_sql;
						...
						select max(matched_count) into v_count from recon_tmp_t1themematch;
						set v_count = ifnull(v_count,0);

						truncate recon_tmp_t1themepseudorows;

						if v_count >= 2 then
							insert into recon_tmp_t1themepseudorows select row from pseudo_rows1 where row <= v_count;
						else
							insert into recon_tmp_t1themepseudorows select 0 union select 1;
						end if;

						insert into recon_tmp_t1themematchdtl (match_gid,parent_tran_gid,parent_tranbrkp_gid,tran_gid,tranbrkp_gid,ko_value,tran_mult,src_comp_flag)
							select
                match_gid,
								tran_gid as parent_tran_gid,
								tranbrkp_gid as parent_tranbrkp_gid,
								JSON_UNQUOTE(JSON_EXTRACT(recon_tmp_t1themematch.matched_json, CONCAT('$[', recon_tmp_t1themepseudorows.row, '].tran_gid'))) AS tran_gid,
								JSON_UNQUOTE(JSON_EXTRACT(recon_tmp_t1themematch.matched_json, CONCAT('$[', recon_tmp_t1themepseudorows.row, '].tranbrkp_gid'))) AS tranbrkp_gid,
								JSON_UNQUOTE(JSON_EXTRACT(recon_tmp_t1themematch.matched_json, CONCAT('$[', recon_tmp_t1themepseudorows.row, '].ko_value'))) AS ko_value,
								JSON_UNQUOTE(JSON_EXTRACT(recon_tmp_t1themematch.matched_json, CONCAT('$[', recon_tmp_t1themepseudorows.row, '].tran_mult'))) AS tran_mult,
								JSON_UNQUOTE(JSON_EXTRACT(recon_tmp_t1themematch.matched_json, CONCAT('$[', recon_tmp_t1themepseudorows.row, '].src_comp_flag'))) AS src_comp_flag
							FROM recon_tmp_t1themematch
							JOIN recon_tmp_t1themepseudorows
							where group_flag = 'M'
							HAVING tran_gid IS NOT NULL;

						-- clear matched records
						truncate recon_tmp_t1themetrangid;

						insert into recon_tmp_t1themetrangid (match_gid,tran_gid)
							select distinct match_gid,tran_gid from recon_tmp_t1themematchdtl where tran_gid > 0 and tranbrkp_gid = 0;

						update recon_tmp_t1themesource as a
            inner join recon_tmp_t1themetrangid as b on a.tran_gid = b.tran_gid
            set
              a.match_flag = 'M'
            where a.match_flag = 'N';

						update recon_tmp_t1themecomparison as a
            inner join recon_tmp_t1themetrangid as b on a.tran_gid = b.tran_gid
            set
              a.match_flag = 'M'
            where a.match_flag = 'N';

						truncate recon_tmp_t1themetranbrkpgid;

						insert into recon_tmp_t1themetranbrkpgid (match_gid,tranbrkp_gid)
							select distinct match_gid,tranbrkp_gid from recon_tmp_t1themematchdtl where tranbrkp_gid > 0;

						update recon_tmp_t1themesource as a
            inner join recon_tmp_t1themetranbrkpgid as b on a.tranbrkp_gid = b.tranbrkp_gid
            set
              a.match_flag = 'M'
            where a.match_flag = 'N';
           ...
           set v_manytomany_match_flag = 'N';
            ...
            set v_match_sql = 'insert into recon_tmp_t1themematch (group_flag,tran_gid,tranbrkp_gid,matched_count,matched_value,tran_mult,matched_json) ';
          ...
          insert ignore into recon_tmp_t1themematchdup (tran_gid,tranbrkp_gid,rec_count)
            select tran_gid,tranbrkp_gid,count(*) from recon_tmp_t1themematchdtl
          ...
          insert ignore into recon_tmp_t1themematchparentgid(parent_tran_gid,parent_tranbrkp_gid)
            select b.parent_tran_gid,b.parent_tranbrkp_gid from recon_tmp_t1themematchdup as a
            inner join recon_tmp_t1themematchdtl as b on a.tran_gid = b.tran_gid and a.tranbrkp_gid = b.tranbrkp_gid
            where src_comp_flag = 'S'
          ...
          update recon_tmp_t1themematch as a
          inner join recon_tmp_t1themematchparentgid as b on a.tran_gid = b.parent_tran_gid and a.tranbrkp_gid = b.parent_tranbrkp_gid
          set a.dup_flag = 'Y';

          update recon_tmp_t1themematch set
            matched_value = abs(matched_value),
            ko_flag = 'Y'
          where dup_flag = 'N';

          -- delete duplicate records in recon_tmp_t1themematchdtl
          delete from recon_tmp_t1themematchdtl
          ...
          (
            select
              parent_tran_gid,
              parent_tranbrkp_gid
            from recon_tmp_t1themematchparentgid
          );

					-- update matched records
					truncate recon_tmp_t1themetrangid;

					insert into recon_tmp_t1themetrangid (tran_gid,match_gid)
						select distinct tran_gid,match_gid from recon_tmp_t1themematchdtl where tran_gid > 0 and tranbrkp_gid = 0;
          ...
          and a.match_gid = 0;
					...
					insert into recon_tmp_t1themetranbrkpgid (tranbrkp_gid,match_gid)
						select distinct tranbrkp_gid,match_gid from recon_tmp_t1themematchdtl where tranbrkp_gid > 0;
          ...
          if v_source_aggfunction_field = '' then
            set v_source_aggfunction_field = concat(",'1'");
					...
					set v_sql = concat('insert into recon_tmp_t1themesourceagg (match_gid',v_source_agg_field,',');
					...
					set v_sql = concat(v_sql,'from recon_tmp_t1themesource ');
          ...
          if v_comparison_aggfunction_field = '' then
            set v_comparison_aggfunction_field = concat(",'1'");
					...
					set v_sql = concat('insert into recon_tmp_t1themecomparisonagg (match_gid',v_comparison_agg_field,',');
					set v_sql = concat(v_sql,'rec_count,themeagg_json,src_comp_flag) ');
					...
					set v_sql = concat(v_sql,'from recon_tmp_t1themecomparison ');
          ...
          -- inner join
          set v_sql = concat('insert into recon_tmp_t1themetranagg(rec_count,themeagg_json) ');
          set v_sql = concat(v_sql,'select (a.rec_count+b.rec_count),JSON_MERGE_PRESERVE(a.themeagg_json,b.themeagg_json) ');
          set v_sql = concat(v_sql,'from recon_tmp_t1themesourceagg as a ');
          set v_sql = concat(v_sql,'inner join recon_tmp_t1themecomparisonagg as b on a.match_gid = b.match_gid ');
          ...
          set v_sql = concat(v_sql,'from recon_tmp_t1themesourceagg as a ');
          set v_sql = concat(v_sql,'left join recon_tmp_t1themecomparisonagg as b on a.match_gid = b.match_gid ');
          set v_sql = concat(v_sql,'where 1 = 1 ',v_themeagg_condition);
          ...
          set v_sql = concat(v_sql,'from recon_tmp_t1themesourceagg as a ');
          set v_sql = concat(v_sql,'right join recon_tmp_t1themecomparisonagg as b on a.match_gid = b.match_gid ');
          set v_sql = concat(v_sql,'where 1 = 1 ',v_themeagg_condition);
          ...
          select max(rec_count) into v_count from recon_tmp_t1themetranagg;
          set v_count = ifnull(v_count,0);
          ...
          truncate recon_tmp_t1themetranwithbrkpgid;

					insert ignore into recon_tmp_t1themetranwithbrkpgid (tran_gid,tranbrkp_gid)
						select
							JSON_UNQUOTE(JSON_EXTRACT(recon_tmp_t1themetranagg.themeagg_json, CONCAT('$[', recon_tmp_t1themepseudorows.row, '].tran_gid'))) AS tran_gid,
							JSON_UNQUOTE(JSON_EXTRACT(recon_tmp_t1themetranagg.themeagg_json, CONCAT('$[', recon_tmp_t1themepseudorows.row, '].tranbrkp_gid'))) AS tranbrkp_gid
						FROM recon_tmp_t1themetranagg
						JOIN recon_tmp_t1themepseudorows
						where rec_count > 0
						HAVING tran_gid IS NOT NULL;
          ...
          -- tran table
					set v_sql = concat("
						...
						where a.tran_gid in (select b.tran_gid from recon_tmp_t1themetranwithbrkpgid as b
          ...
          -- tranbrkp table
					set v_sql = concat("
						update ",v_tranbrkp_table," as a set
							a.theme_code = concat(if(a.theme_code = '','",v_theme_name,"',
							concat(a.theme_code,',','",v_theme_name,"')))
						where (a.tran_gid,a.tranbrkp_gid) in (select b.tran_gid,b.tranbrkp_gid from recon_tmp_t1themetranwithbrkpgid as b
          ...
          delete from recon_tmp_t1themeindex
          where table_name in ('recon_tmp_t1themesourceagg','recon_tmp_t1themecomparisonagg','recon_tmp_t1themetranagg');

          truncate recon_tmp_t1themesource;
          truncate recon_tmp_t1themecomparison;
          truncate recon_tmp_t1themesourcedup;
          truncate recon_tmp_t1themetrangid;
          truncate recon_tmp_t1themetranbrkpgid;

          truncate recon_tmp_t1themesourceagg;
          truncate recon_tmp_t1themecomparisonagg;
          truncate recon_tmp_t1themetranagg;
