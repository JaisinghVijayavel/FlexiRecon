...
me:BEGIN
  ...
  */
  ...
  drop temporary table if exists recon_tmp_t9match;
  drop temporary table if exists recon_tmp_t9matchdtl;
  drop temporary table if exists recon_tmp_t9matchdtlgid;
  drop temporary table if exists recon_tmp_t9matchdup;
  drop temporary table if exists recon_tmp_t9matchparentgid;
  drop temporary table if exists recon_tmp_t9matchko;
  drop temporary table if exists recon_tmp_t9matchdiff;
  drop temporary table if exists recon_tmp_t9matchdiffdtl;
  drop temporary table if exists recon_tmp_t9manymatch;
  drop temporary table if exists recon_tmp_t9kodtl;
  drop temporary table if exists recon_tmp_t9kodtlsumm;
  drop temporary table if exists recon_tmp_t9pseudorows;
  drop temporary table if exists recon_tmp_t9trangid;
  drop temporary table if exists recon_tmp_t9trangid1;
  drop temporary table if exists recon_tmp_t9trangid2;
  drop temporary table if exists recon_tmp_t9tranbrkpgid;
  drop temporary table if exists recon_tmp_t9tranwithbrkpgid;
  drop temporary table if exists recon_tmp_t9gid;
  drop temporary table if exists recon_tmp_t9tranroundoff;

  drop temporary table if exists recon_tmp_t9thresholddiff;
  drop temporary table if exists recon_tmp_t9thresholddiffdtl;

  drop temporary table if exists recon_tmp_t9index;
  drop temporary table if exists recon_tmp_t9sql;

  CREATE TEMPORARY TABLE recon_tmp_t9index(
  ...
  insert into recon_tmp_t9index select 'recon_tmp_t9source','idx_tran_date','Y';
  insert into recon_tmp_t9index select 'recon_tmp_t9comparison','idx_tran_date','Y';

  CREATE temporary TABLE recon_tmp_t9thresholddiff(
  ...
  create temporary table recon_tmp_t9thresholddiffdtl(
  ...
  -- drop table if exists recon_tmp_t9match;

  CREATE temporary TABLE recon_tmp_t9match(
    tran_gid int unsigned NOT NULL,
    tranbrkp_gid int unsigned not null default 0,
    matched_count int not null default 0,
    matched_value double(15,2) not null default 0,
  ...
  create temporary table recon_tmp_t9matchdtl(
  ...
  CREATE temporary TABLE recon_tmp_t9manymatch(
    tran_gid int unsigned NOT NULL,
    tranbrkp_gid int unsigned not null default 0,
  ...
  create temporary table recon_tmp_t9kodtl(
  ...
  create temporary table recon_tmp_t9kodtlsumm(
  ...
  create temporary table recon_tmp_t9matchdtlgid(
    matchdtl_gid int unsigned NOT NULL,
    PRIMARY KEY (matchdtl_gid)
  ) ENGINE = MyISAM;

  create temporary table recon_tmp_t9matchdup(
  ...
  create temporary table recon_tmp_t9matchparentgid(
    parent_tran_gid int unsigned NOT NULL,
  ...
  create temporary table recon_tmp_t9matchko(
    tran_gid int unsigned NOT NULL,
    ko_value decimal(15,2) not null default 0,
    excp_value decimal(15,2) not null default 0,
  ...
  create temporary table recon_tmp_t9matchdiff(
    tran_gid int unsigned NOT NULL,
    tran_mult tinyint not null default 0,
  ...
  create temporary table recon_tmp_t9matchdiffdtl(
  ...
  CREATE temporary TABLE recon_tmp_t9pseudorows(
    row int unsigned NOT NULL,
    PRIMARY KEY (row)
  ) ENGINE = MyISAM;

  insert into recon_tmp_t9pseudorows select 0 union select 1;

  CREATE temporary TABLE recon_tmp_t9trangid(
    tran_gid int unsigned NOT NULL,
    PRIMARY KEY (tran_gid)
  ) ENGINE = MyISAM;

  CREATE temporary TABLE recon_tmp_t9trangid1(
    tran_gid int unsigned NOT NULL,
    PRIMARY KEY (tran_gid)
  ) ENGINE = MyISAM;

  CREATE temporary TABLE recon_tmp_t9trangid2(
    tran_gid int unsigned NOT NULL,
    PRIMARY KEY (tran_gid)
  ) ENGINE = MyISAM;

  CREATE temporary TABLE recon_tmp_t9tranbrkpgid(
  ...
  CREATE temporary TABLE recon_tmp_t9tranwithbrkpgid(
    tran_gid int unsigned not null,
    tranbrkp_gid int unsigned NOT NULL,
    rec_count int not null default 0,
    PRIMARY KEY (tran_gid,tranbrkp_gid)
  ...
  CREATE temporary TABLE recon_tmp_t9gid(
    tran_gid int unsigned NOT NULL,
    tranbrkp_gid int unsigned NOT NULL default 0,
    PRIMARY KEY (tran_gid,tranbrkp_gid)
  ) ENGINE = MyISAM;

  CREATE temporary TABLE recon_tmp_t9tranroundoff(
    tran_gid int unsigned NOT NULL,
    tranbrkp_gid int unsigned NOT NULL,
  ...
  CREATE temporary TABLE recon_tmp_t9sql(
  ...
  */
  ...
  applyrule_block:begin
    ...
    applyrule_loop: loop
      ...
      set v_source_head_sql = concat('insert into recon_tmp_t9source (',v_tran_fields,') ');
      ...
      set v_comparison_head_sql = concat('insert into recon_tmp_t9comparison (',v_tran_fields,') ');
      ...
      set v_source_headbrkp_sql = concat('insert into recon_tmp_t9source (',v_tranbrkp_fields,') ');
      ...
      set v_comparison_headbrkp_sql = concat('insert into recon_tmp_t9comparison (',v_tranbrkp_fields,') ');
      ...
      if v_recontype_code <> 'N' then
        ...
        set v_comparison_headbrkp_sql = concat(v_comparison_headbrkp_sql,' and 1 = 2 ');
          ...
          drop temporary table if exists recon_tmp_t9source;
          drop temporary table if exists recon_tmp_t9comparison;
          drop temporary table if exists recon_tmp_t9sourcedup;

          /*
          drop table if exists recon_tmp_t9source;
          drop table if exists recon_tmp_t9comparison;
          */
          create temporary table recon_tmp_t9source select * from recon_trn_ttranwithbrkp where 1 = 2;
          alter table recon_tmp_t9source add primary key(tran_gid,tranbrkp_gid);
          create index idx_excp_value on recon_tmp_t9source(excp_value);
          create index idx_tran_date on recon_tmp_t9source(tran_date);
          create index idx_tran_gid on recon_tmp_t9source(tran_gid);
          create index idx_tranbrkp_gid on recon_tmp_t9source(tranbrkp_gid);
          create index idx_dataset_code on recon_tmp_t9source(recon_code,dataset_code);
          alter table recon_tmp_t9source ENGINE = MyISAM;

          create temporary table recon_tmp_t9comparison select * from recon_trn_ttranwithbrkp where 1 = 2;
          alter table recon_tmp_t9comparison add primary key(tran_gid,tranbrkp_gid);
          create index idx_excp_value on recon_tmp_t9comparison(excp_value);
          create index idx_tran_date on recon_tmp_t9comparison(tran_date);
          create index idx_tran_gid on recon_tmp_t9comparison(tran_gid);
          create index idx_tranbrkp_gid on recon_tmp_t9comparison(tranbrkp_gid);
          create index idx_dataset_code on recon_tmp_t9comparison(recon_code,dataset_code);
          alter table recon_tmp_t9comparison ENGINE = MyISAM;

          create temporary table recon_tmp_t9sourcedup select * from recon_trn_ttranwithbrkp where 1 = 2;
          alter table recon_tmp_t9sourcedup add primary key(tran_gid,tranbrkp_gid);
          create index idx_excp_value on recon_tmp_t9sourcedup(excp_value);
          create index idx_tran_date on recon_tmp_t9sourcedup(tran_date);
          create index idx_dataset_code on recon_tmp_t9sourcedup(recon_code,dataset_code);
          alter table recon_tmp_t9sourcedup ENGINE = MyISAM;

          delete from recon_tmp_t9index where index_name <> 'idx_tran_date';
          truncate recon_tmp_t9sql;
          ...
          rule_block:begin
            ...
            rule_loop: loop
              ...
              if not exists(select index_name from recon_tmp_t9index
                            WHERE table_name = 'recon_tmp_t9source'
                            and index_name = v_index_name) then

                if substr(v_source_field,1,3) = 'col' then
                  set v_sql = concat('alter table recon_tmp_t9source modify column ',v_source_field,' varchar(255) default null');
                  call pr_run_sql(v_sql,@msg,@result);

                  set v_index_sql = concat('create index idx_',v_source_field,' on recon_tmp_t9source(',v_source_field,'(255))');
                else
                  set v_index_sql = concat('create index idx_',v_source_field,' on recon_tmp_t9source(',v_source_field,')');
                end if;

                call pr_run_sql(v_index_sql,@msg,@result);

                insert into recon_tmp_t9index(table_name,index_name) select 'recon_tmp_t9source',v_index_name;
              end if;

              set v_index_name = concat('idx_',v_comparison_field);

              if not exists(select index_name from recon_tmp_t9index
                            WHERE table_name = 'recon_tmp_t9comparison'
                            and index_name = v_index_name) then

                if substr(v_comparison_field,1,3) = 'col' then
                  set v_sql = concat('alter table recon_tmp_t9comparison modify column ',v_source_field,' varchar(255) default null');
                  call pr_run_sql(v_sql,@msg,@result);

                  set v_index_sql = concat('create index idx_',v_comparison_field,' on recon_tmp_t9comparison(',v_comparison_field,'(255))');
                else
                  set v_index_sql = concat('create index idx_',v_comparison_field,' on recon_tmp_t9comparison(',v_comparison_field,')');
                end if;

                call pr_run_sql(v_index_sql,@msg,@result);

                insert into recon_tmp_t9index(table_name,index_name) select 'recon_tmp_t9comparison',v_index_name;
              end if;
              ...
              if (instr(v_extraction_criteria,'$FIELD$') > 0 or v_extraction_filter > 0)
                ...
                set v_sql = '';
                set v_sql = concat(v_sql,'update recon_tmp_t9source set ');
                set v_sql = concat(v_sql,v_field,'=',v_field_format);

                insert into recon_tmp_t9sql(table_type,tran_acc_mode,sql_query) values ('S',v_source_acc_mode,v_sql);
              ...
              if (instr(v_comparison_criteria,'$FIELD$') > 0 or v_comparison_filter > 0)
                ...
                set v_sql = '';
                set v_sql = concat(v_sql,'update recon_tmp_t9comparison set ');
                set v_sql = concat(v_sql,v_field,'=',v_field_format,' ');
                set v_sql = concat(v_sql,'where tran_acc_mode =',char(39), v_comparison_acc_mode,char(39), ' ');

                insert into recon_tmp_t9sql(table_type,tran_acc_mode,sql_query) values ('C',v_comparison_acc_mode,v_sql);
                ...
                insert into recon_tmp_t9sql(table_type,tran_acc_mode,sql_query) values ('C',v_source_acc_mode,v_sql);
          ...
          truncate recon_tmp_t9source;
          truncate recon_tmp_t9comparison;
          ...
          */
          ...
          update recon_tmp_t9source set excp_value = (excp_value - roundoff_value * tran_mult);
          ...
          */
          ...
          update recon_tmp_t9comparison set excp_value = (excp_value - roundoff_value * tran_mult);
          ...
          */
          ...
          sql_block:begin
            ...
            select sql_query from recon_tmp_t9sql
               where table_type = 'S'
          ...
          if in_automatch_flag = 'N' then
            ...
            delete from recon_tmp_t9source where (tran_gid,tranbrkp_gid) in
            (
            ...
            delete from recon_tmp_t9comparison where (tran_gid,tranbrkp_gid) in
            (
              select tran_gid,tranbrkp_gid from recon_trn_tpreviewdtl
              where job_gid = in_job_gid
              and delete_flag = 'N'
          ...
          -- remove duplicate
          set v_trangid_sql = 'insert into recon_tmp_t9trangid ';
          set v_trangid_sql = concat(v_trangid_sql,'select cast(group_concat(tran_gid) as unsigned) from recon_tmp_t9source as a ') ;
          ...
          set v_trangid_sql = 'insert into recon_tmp_t9tranbrkpgid (tranbrkp_gid) ';
          set v_trangid_sql = concat(v_trangid_sql,'select cast(group_concat(tranbrkp_gid) as unsigned) from recon_tmp_t9source as a ') ;
          -- set v_trangid_sql = concat(v_trangid_sql,' where dataset_code = ',char(39),v_source_dataset_code,char(39));
          set v_trangid_sql = concat(v_trangid_sql,' where 1 = 1 ');
          ...
          insert into recon_tmp_t9sourcedup select * from recon_tmp_t9source where tran_gid not in (select tran_gid from recon_tmp_t9trangid) and tranbrkp_gid = 0;
          insert into recon_tmp_t9sourcedup select * from recon_tmp_t9source where tranbrkp_gid not in (select tranbrkp_gid from recon_tmp_t9tranbrkpgid) and tranbrkp_gid > 0;

          delete from recon_tmp_t9source where tran_gid not in (select tran_gid from recon_tmp_t9trangid) and tranbrkp_gid = 0;
          delete from recon_tmp_t9source where tranbrkp_gid not in (select tranbrkp_gid from recon_tmp_t9tranbrkpgid) and tranbrkp_gid > 0;

          truncate recon_tmp_t9trangid;
          truncate recon_tmp_t9tranbrkpgid;


          -- find matchoff diff value
          -- plus value
          set v_match_sql = 'insert into recon_tmp_t9thresholddiff (group_flag,tran_gid,tranbrkp_gid,matched_count,diff_value,matched_json) ';
          ...
          set v_match_sql = concat(v_match_sql,'from recon_tmp_t9source as a ');
          set v_match_sql = concat(v_match_sql,'inner join recon_tmp_t9comparison as b ');
          ...
          -- minus value
          set v_match_sql = 'insert ignore into recon_tmp_t9thresholddiff (group_flag,tran_gid,tranbrkp_gid,matched_count,diff_value,matched_json) ';
          set v_match_sql = concat(v_match_sql,'select ',char(39),'N',char(39),',');
          set v_match_sql = concat(v_match_sql,'a.tran_gid,a.tranbrkp_gid,count(*) as matched_count,(a.excp_value-b.excp_value) as diff_value,');
          set v_match_sql = concat(v_match_sql,'cast(concat(',char(39),'[');
          set v_match_sql = concat(v_match_sql,'{');
          ...
          truncate recon_tmp_t9pseudorows;
          insert into recon_tmp_t9pseudorows select 0 union select 1;

          insert into recon_tmp_t9thresholddiffdtl (parent_tran_gid,parent_tranbrkp_gid,tran_gid,tranbrkp_gid,ko_value,src_comp_flag,tran_acc_mode)
            select
              tran_gid as parent_tran_gid,
              tranbrkp_gid as parent_tranbrkp_gid,
              JSON_UNQUOTE(JSON_EXTRACT(recon_tmp_t9thresholddiff.matched_json, CONCAT('$[', recon_tmp_t9pseudorows.row, '].tran_gid'))) AS tran_gid,
              JSON_UNQUOTE(JSON_EXTRACT(recon_tmp_t9thresholddiff.matched_json, CONCAT('$[', recon_tmp_t9pseudorows.row, '].tranbrkp_gid'))) AS tranbrkp_gid,
              JSON_UNQUOTE(JSON_EXTRACT(recon_tmp_t9thresholddiff.matched_json, CONCAT('$[', recon_tmp_t9pseudorows.row, '].ko_value'))) AS ko_value,
              JSON_UNQUOTE(JSON_EXTRACT(recon_tmp_t9thresholddiff.matched_json, CONCAT('$[', recon_tmp_t9pseudorows.row, '].src_comp_flag'))) AS src_comp_flag,
              JSON_UNQUOTE(JSON_EXTRACT(recon_tmp_t9thresholddiff.matched_json, CONCAT('$[', recon_tmp_t9pseudorows.row, '].tran_acc_mode'))) AS tran_acc_mode
            FROM recon_tmp_t9thresholddiff
            JOIN recon_tmp_t9pseudorows
            where group_flag = 'N'
            HAVING tran_gid IS NOT NULL;

          if v_group_flag = 'Y' then
            -- plus value
            set v_match_sql = 'insert ignore into recon_tmp_t9thresholddiff (group_flag,tran_gid,tranbrkp_gid,matched_count,diff_value,tran_mult,matched_json) ';
            ...
            insert ignore into recon_tmp_t9gid select tran_gid,tranbrkp_gid from recon_tmp_t9thresholddiff;
            ...
            select max(matched_count) into v_count from recon_tmp_t9thresholddiff;
            set v_count = ifnull(v_count,0);

            truncate recon_tmp_t9pseudorows;

            if v_count >= 2 then
              insert into recon_tmp_t9pseudorows select row from pseudo_rows1 where row <= v_count;
            else
              insert into recon_tmp_t9pseudorows select 0 union select 1;
            end if;
          ...
          truncate recon_tmp_t9matchdup;
          truncate recon_tmp_t9matchparentgid;

          -- duplicate validation
          insert into recon_tmp_t9matchdup (tran_gid,tranbrkp_gid,rec_count)
            select tran_gid,tranbrkp_gid,count(*) from recon_tmp_t9thresholddiffdtl
            group by tran_gid,tranbrkp_gid
            having count(*) > 1;

          insert into recon_tmp_t9matchparentgid(parent_tran_gid,parent_tranbrkp_gid)
            select b.parent_tran_gid,b.parent_tranbrkp_gid from recon_tmp_t9matchdup as a
            inner join recon_tmp_t9thresholddiffdtl as b on a.tran_gid = b.tran_gid and a.tranbrkp_gid = b.tranbrkp_gid
            group by b.parent_tran_gid,b.parent_tranbrkp_gid;

          update recon_tmp_t9thresholddiff as a
          inner join recon_tmp_t9matchparentgid as b on a.tran_gid = b.parent_tran_gid and a.tranbrkp_gid = b.parent_tranbrkp_gid
          set a.dup_flag = 'Y';

          update recon_tmp_t9thresholddiff as a
          inner join recon_tmp_t9thresholddiffdtl as b on a.tran_gid = b.tran_gid
          ...
          source_block:begin
            ...
            select tran_gid,tranbrkp_gid,tran_acc_mode,diff_value,matched_count from recon_tmp_t9thresholddiff
               where dup_flag = 'N';
            ...
            source_loop: loop
              ...
              if v_diff_value > 0 then
                ...
                comparison_block:begin
                  ...
                  select tran_gid,tranbrkp_gid,ko_value from recon_tmp_t9thresholddiffdtl
                  ...
                  comparison_loop: loop
                    ...
                    if v_ko_value >= v_diff_value then
                      update recon_tmp_t9comparison set
                        excp_value = excp_value - v_diff_value
                      where tran_gid = v_tran_gid
                      and tranbrkp_gid = v_tranbrkp_gid;
          ...
          truncate recon_tmp_t9thresholddiff;
          truncate recon_tmp_t9thresholddiffdtl;
          ...
          select * from recon_tmp_t9thresholddiff;
          leave me;

					-- many to many match
					if v_manytomany_match_flag = 'Y' then
						set v_match_sql = 'insert ignore into recon_tmp_t9manymatch (tran_gid,tranbrkp_gid,matched_count,';
            ...
            if v_recorder <> '' then
              set v_match_sql = concat(v_match_sql,v_recorder);
						...
						set v_match_sql = 'insert into recon_tmp_t9match (group_flag,tran_gid,tranbrkp_gid,matched_count,matched_value,tran_mult,matched_json) ';
						...
						set v_match_sql = concat(v_match_sql,'from recon_tmp_t9manymatch ');
              ...
              if (v_recontype_code <> 'I' and v_recontype_code <> 'V') or v_reversal_flag = 'Y' then
                ...
                -- mirror
						...
						select max(matched_count) into v_count from recon_tmp_t9match;
						set v_count = ifnull(v_count,0);
						...
						insert into recon_tmp_t9matchdtl (parent_tran_gid,parent_tranbrkp_gid,tran_gid,tranbrkp_gid,ko_value,tran_mult,src_comp_flag)
							select
								tran_gid as parent_tran_gid,
								tranbrkp_gid as parent_tranbrkp_gid,
								JSON_UNQUOTE(JSON_EXTRACT(recon_tmp_t9match.matched_json, CONCAT('$[', recon_tmp_t9pseudorows.row, '].tran_gid'))) AS tran_gid,
								JSON_UNQUOTE(JSON_EXTRACT(recon_tmp_t9match.matched_json, CONCAT('$[', recon_tmp_t9pseudorows.row, '].tranbrkp_gid'))) AS tranbrkp_gid,
								JSON_UNQUOTE(JSON_EXTRACT(recon_tmp_t9match.matched_json, CONCAT('$[', recon_tmp_t9pseudorows.row, '].ko_value'))) AS ko_value,
								JSON_UNQUOTE(JSON_EXTRACT(recon_tmp_t9match.matched_json, CONCAT('$[', recon_tmp_t9pseudorows.row, '].tran_mult'))) AS tran_mult,
								JSON_UNQUOTE(JSON_EXTRACT(recon_tmp_t9match.matched_json, CONCAT('$[', recon_tmp_t9pseudorows.row, '].src_comp_flag'))) AS src_comp_flag
							FROM recon_tmp_t9match
							JOIN recon_tmp_t9pseudorows
							where group_flag = 'M'
							HAVING tran_gid IS NOT NULL;
						...
						insert into recon_tmp_t9trangid
							select distinct tran_gid from recon_tmp_t9matchdtl where tran_gid > 0 and tranbrkp_gid = 0;

						delete a.* from recon_tmp_t9source as a
            where a.tran_gid in (select b.tran_gid from recon_tmp_t9trangid as b where a.tran_gid = b.tran_gid);

						delete a.* from recon_tmp_t9comparison as a
            where a.tran_gid in (select b.tran_gid from recon_tmp_t9trangid as b where a.tran_gid = b.tran_gid);

						truncate recon_tmp_t9tranbrkpgid;

						insert into recon_tmp_t9tranbrkpgid (tranbrkp_gid)
							select distinct tranbrkp_gid from recon_tmp_t9matchdtl where tranbrkp_gid > 0;

						delete a.* from recon_tmp_t9source as a
            where a.tranbrkp_gid in (select b.tranbrkp_gid from recon_tmp_t9tranbrkpgid as b
          ...
          if v_manytomany_match_flag = 'N' then
						set v_match_sql = 'insert into recon_tmp_t9match (group_flag,tran_gid,tranbrkp_gid,matched_count,matched_value,matched_json) ';
						set v_match_sql = concat(v_match_sql,'select ',char(39),'N',char(39),',');
						set v_match_sql = concat(v_match_sql,'a.tran_gid,a.tranbrkp_gid,count(*) as matched_count,');
          ...
          -- duplicate validation
          insert into recon_tmp_t9matchdup (tran_gid,tranbrkp_gid,rec_count)
            select tran_gid,tranbrkp_gid,count(*) from recon_tmp_t9matchdtl
            group by tran_gid,tranbrkp_gid
            having count(*) > 1;

          insert into recon_tmp_t9matchparentgid(parent_tran_gid,parent_tranbrkp_gid)
            select b.parent_tran_gid,b.parent_tranbrkp_gid from recon_tmp_t9matchdup as a
            inner join recon_tmp_t9matchdtl as b on a.tran_gid = b.tran_gid and a.tranbrkp_gid = b.tranbrkp_gid
            group by b.parent_tran_gid,b.parent_tranbrkp_gid;

          update recon_tmp_t9match as a
          inner join recon_tmp_t9matchparentgid as b on a.tran_gid = b.parent_tran_gid and a.tranbrkp_gid = b.parent_tranbrkp_gid
          set a.dup_flag = 'Y';

          update recon_tmp_t9match set
            matched_value = abs(matched_value),
            ko_flag = 'Y'
          where dup_flag = 'N';

          if in_automatch_flag = 'Y' then
            truncate recon_tmp_t9matchko;

            if v_recontype_code <> 'N' then
              -- match diff
							set v_sql = concat("
              insert into recon_tmp_t9matchdiff(tran_gid,tran_value,excp_value,mapped_value,tran_mult,diff_value)
              select
                a.tran_gid,b.tran_value,b.excp_value,b.mapped_value,b.tran_mult,
                b.excp_value - sum(a.ko_value*a.tran_mult)*b.tran_mult
              from recon_tmp_t9match as m
              inner join recon_tmp_t9matchdtl as a on m.tran_gid = a.parent_tran_gid and m.tranbrkp_gid = a.parent_tranbrkp_gid
              ...
              if exists(select * from recon_tmp_t9matchdiff) then
								-- diff block
								diff_block:begin
									declare diff_done int default 0;
									declare diff_cursor cursor for
									select tran_gid,tran_mult,mapped_value,diff_value from recon_tmp_t9matchdiff;
                    ...
                    if v_mapped_value > 0 then
                      ...
                      from
                      (
                        select tranbrkp_gid,excp_value,tran_mult from recon_tmp_t9source
                        where tran_gid = v_tran_gid
                        and tranbrkp_gid > 0

                        union

                        select tranbrkp_gid,excp_value,tran_mult from recon_tmp_t9comparison
                        where tran_gid = v_tran_gid
                        and tranbrkp_gid > 0
                      ) as a;
                        ...
                        set v_sql = concat("
													...
													(
														...
														(
															select
																a.tranbrkp_gid
															from recon_tmp_t9matchdtl as a
															inner join recon_tmp_t9match as m on m.tran_gid = a.parent_tran_gid and m.tranbrkp_gid = a.parent_tranbrkp_gid
                      ...
                      -- or (v_excp_value < v_diff_value and v_diff_value <> 0) then
												...
												into
													...
													v_matched_value
												from recon_tmp_t9matchdtl as a
												inner join recon_tmp_t9match as b on a.parent_tran_gid = b.tran_gid and a.parent_tranbrkp_gid = b.tranbrkp_gid
												...
												insert ignore into recon_tmp_t9matchdiffdtl
													select * from recon_tmp_t9matchdtl
                ...
                if exists(select * from recon_tmp_t9match
                  where ko_flag = 'D') then
                  truncate recon_tmp_t9trangid;

                  insert into recon_tmp_t9trangid(tran_gid)
                    select tran_gid from recon_tmp_t9matchdiffdtl
                    group by tran_gid
                    having count(*) > 1;

                  insert into recon_tmp_t9tranwithbrkpgid(tran_gid,tranbrkp_gid,rec_count)
                    select a.parent_tran_gid,a.parent_tranbrkp_gid,count(*) from recon_tmp_t9matchdiffdtl as a
                    inner join recon_tmp_t9trangid as b on a.tran_gid = b.tran_gid
                    group by a.parent_tran_gid,a.parent_tranbrkp_gid;

                  update recon_tmp_t9match as a
                  inner join recon_tmp_t9tranwithbrkpgid as b on a.tran_gid = b.tran_gid and a.tranbrkp_gid = b.tranbrkp_gid
                  set
                    a.dup_flag = 'N',
                    a.ko_flag = 'Y'
                  where b.rec_count = 1
              ...
              update recon_tmp_t9match as a
              inner join recon_tmp_t9matchdtl as b on a.tran_gid = b.parent_tran_gid and a.tranbrkp_gid = b.parent_tranbrkp_gid
              ...
              -- knockoff validation
							set v_sql = concat("
								insert into recon_tmp_t9matchko (tran_gid,ko_value,excp_value)
								select
              ...
              update recon_tmp_t9matchdtl as a
              inner join recon_tmp_t9matchko as b on a.tran_gid = b.tran_gid
              set a.ko_flag = 'Y';
            ...
            -- insert into knockoff
						...
						set v_sql = concat("
							insert into recon_tmp_t9kodtl
							( ko_gid,tran_gid,tranbrkp_gid,ko_value,tran_mult)
							select
								ko_gid,
								JSON_UNQUOTE(JSON_EXTRACT(",v_ko_table,".kodtl_json, CONCAT('$[', recon_tmp_t9pseudorows.row, '].tran_gid'))) AS tran_gid,
								JSON_UNQUOTE(JSON_EXTRACT(",v_ko_table,".kodtl_json, CONCAT('$[', recon_tmp_t9pseudorows.row, '].tranbrkp_gid'))) AS tranbrkp_gid,
								JSON_UNQUOTE(JSON_EXTRACT(",v_ko_table,".kodtl_json, CONCAT('$[', recon_tmp_t9pseudorows.row, '].ko_value'))) AS excp_value,
								JSON_UNQUOTE(JSON_EXTRACT(",v_ko_table,".kodtl_json, CONCAT('$[', recon_tmp_t9pseudorows.row, '].tran_mult'))) AS tran_mult
							FROM ",v_ko_table,"
							JOIN recon_tmp_t9pseudorows
            ...
            -- insert in kodtl table
						set v_sql = concat("
							insert into ",v_kodtl_table," (ko_gid,tran_gid,tranbrkp_gid,ko_value,ko_mult)
								select ko_gid,tran_gid,tranbrkp_gid,ko_value,tran_mult from recon_tmp_t9kodtl");
            ...
            if v_recontype_code <> 'N' then
              -- insert in ko roundoff table
							set v_sql = concat("
								insert into ",v_koroundoff_table," (ko_gid,tran_gid,tranbrkp_gid,roundoff_value)
									...
									from recon_tmp_t9tranroundoff as a
									inner join recon_tmp_t9kodtl as b on a.tran_gid = b.tran_gid and a.tranbrkp_gid = b.tranbrkp_gid");
									
							call pr_run_sql(v_sql,@msg,@result);

              insert into recon_tmp_t9kodtlsumm (max_ko_gid,tran_gid,ko_value,rec_count)
              select
                max(ko_gid) as max_ko_gid,tran_gid,sum(ko_value*tran_mult) as ko_value,count(*) as rec_count
              from recon_tmp_t9kodtl
              group by tran_gid;

              -- roundoff tranbrkp_gid = 0 cases
              update recon_tmp_t9kodtlsumm as a
              inner join recon_tmp_t9tranroundoff as b on a.tran_gid = b.tran_gid
                and b.tranbrkp_gid = 0
              ...
              inner join
              (
                select tran_gid,sum(roundoff_value) as roundoff_value from recon_tmp_t9tranroundoff
              ...
              -- update in tran table
							set v_sql = concat("
								...
								inner join recon_tmp_t9kodtlsumm as b on a.tran_gid = b.tran_gid
              ...
              -- update in tranbrkp table
							set v_sql = concat("
								...
								inner join recon_tmp_t9kodtl as b on a.tranbrkp_gid = b.tranbrkp_gid
              ...
              -- update in tranbrkp table
							set v_sql = concat("
								update ",v_tranbrkp_table," as a
								inner join recon_tmp_t9tranroundoff as r on a.tranbrkp_gid = r.tranbrkp_gid
								inner join recon_tmp_t9kodtl as b on a.tranbrkp_gid = b.tranbrkp_gid
            ...
            truncate recon_tmp_t9trangid;
						...
						set v_sql = concat("
							insert into recon_tmp_t9trangid
								select a.tran_gid from recon_tmp_t9kodtlsumm as a
								inner join ",v_tran_table," as b on a.tran_gid = b.tran_gid
									and b.excp_value = 0
									and b.mapped_value = 0
									and b.delete_flag = 'N'");
						...
						set v_sql = concat("
							insert into ",v_tranko_table,"
								select t.* from recon_tmp_t9trangid as g
            ...
            insert into recon_tmp_t9tranbrkpgid (tranbrkp_gid) select tranbrkp_gid from recon_tmp_t9kodtl where tranbrkp_gid > 0;

						set v_sql = concat("
							insert into ",v_tranbrkpko_table,"
								select b.* from recon_tmp_t9tranbrkpgid as g
								inner join ",v_tranbrkp_table," as b on g.tranbrkp_gid = b.tranbrkp_gid");

						call pr_run_sql(v_sql,@msg,@result);

						set v_sql = concat("delete a.* from ",v_tranbrkp_table," as a
							where a.tranbrkp_gid in (select b.tranbrkp_gid from recon_tmp_t9tranbrkpgid as b where a.tranbrkp_gid = b.tranbrkp_gid)");
            ...
            truncate recon_tmp_t9trangid;
						...
						set v_sql = concat("
							insert into recon_tmp_t9trangid
								select distinct a.tran_gid from recon_tmp_t9tranbrkpgid as a
            ...
            */
            ...
            truncate recon_tmp_t9trangid1;
            truncate recon_tmp_t9trangid2;

            -- set tran_gid1
						set v_sql = concat("
							insert into recon_tmp_t9trangid1
								select distinct tran_gid from recon_tmp_t9tranbrkpgid");

						call pr_run_sql(v_sql,@msg,@result);

            -- set tran_gid2
						set v_sql = concat("
							insert into recon_tmp_t9trangid2
            ...
            -- move tran mapped_value > 0
						set v_sql = concat("
							insert into recon_tmp_t9trangid
								select distinct a.tran_gid from recon_tmp_t9trangid1 as a
								left join recon_tmp_t9trangid2 as b on a.tran_gid = b.tran_gid
								where b.tran_gid is null");

						call pr_run_sql(v_sql,@msg,@result);

            -- keep excp value non-zero cases
            truncate recon_tmp_t9gid;

						set v_sql = concat("
							insert into recon_tmp_t9gid(tran_gid) select tran_gid from ",v_tran_table," as a
								where a.tran_gid in (select b.tran_gid from recon_tmp_t9trangid as b where a.tran_gid = b.tran_gid)
								and a.excp_value <> 0
								and a.delete_flag = 'N'");
            ...
            delete a.* from recon_tmp_t9trangid as a
            where a.tran_gid in (select b.tran_gid from recon_tmp_t9gid as b where a.tran_gid = b.tran_gid);
            ...
            select
              recon_tmp_t9pseudorows.row+1,
              preview_gid,
              job_gid,
              JSON_UNQUOTE(JSON_EXTRACT(recon_trn_tpreview.previewdtl_json, CONCAT('$[', recon_tmp_t9pseudorows.row, '].tran_gid'))) AS tran_gid,
              JSON_UNQUOTE(JSON_EXTRACT(recon_trn_tpreview.previewdtl_json, CONCAT('$[', recon_tmp_t9pseudorows.row, '].tranbrkp_gid'))) AS tranbrkp_gid,
              JSON_UNQUOTE(JSON_EXTRACT(recon_trn_tpreview.previewdtl_json, CONCAT('$[', recon_tmp_t9pseudorows.row, '].ko_value'))) AS excp_value,
              JSON_UNQUOTE(JSON_EXTRACT(recon_trn_tpreview.previewdtl_json, CONCAT('$[', recon_tmp_t9pseudorows.row, '].tran_mult'))) AS tran_mult,
              v_reversal_flag,
              JSON_UNQUOTE(JSON_EXTRACT(recon_trn_tpreview.previewdtl_json, CONCAT('$[', recon_tmp_t9pseudorows.row, '].src_comp_flag'))) AS src_comp_flag
            ...
            if v_recontype_code <> 'N' then
              ...
              update recon_tmp_ttranbrkp as a
							inner join recon_tmp_t9matchdtl as c on a.tranbrkp_gid = c.tranbrkp_gid and a.tran_gid = c.tran_gid
          ...
          truncate recon_tmp_t9source;
          truncate recon_tmp_t9comparison;
          truncate recon_tmp_t9sourcedup;
          truncate recon_tmp_t9match;
          truncate recon_tmp_t9matchdtl;
          truncate recon_tmp_t9matchdtlgid;
          truncate recon_tmp_t9matchdup;
          truncate recon_tmp_t9matchparentgid;
          truncate recon_tmp_t9matchko;
          truncate recon_tmp_t9manymatch;
          truncate recon_tmp_t9kodtl;
          truncate recon_tmp_t9kodtlsumm;
          truncate recon_tmp_t9trangid;
          truncate recon_tmp_t9tranbrkpgid;
