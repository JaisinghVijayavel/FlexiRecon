...
DROP PROCEDURE IF EXISTS `pr_run_reconrulechk` $$
CREATE PROCEDURE `pr_run_reconrulechk`(
...
me:BEGIN
  ...
  rule_block:begin
    ...
    rule_loop: loop
      ...
      if v_rule_code = 'RU2032' then
        select v_recon_code,v_rule_code,v_group_flag,v_job_gid,in_period_from,in_period_to,v_system_match_flag,v_rule_apply_on;

      if v_rule_apply_on = 'T' then
        call pr_run_automatchchk2(v_recon_code,v_rule_code,v_group_flag,v_job_gid,in_period_from,in_period_to,v_system_match_flag,in_user_code,@msg,@result);
