...
DROP PROCEDURE IF EXISTS `pr_get_kosummary_nonvaluebase` $$
CREATE PROCEDURE `pr_get_kosummary_nonvaluebase`(
  ...
  in in_dataset_formt varchar(32),
  out out_msg text,
  out out_result int
)
me:BEGIN
  ...
  */
  ...
  -- return result
  select
    a.`Row Labels`,sum(a.`Dr Count` + a.`Cr Count`) as Count,
  ...
  order by recon_code,dataset_code,rec_slno) as a
  group by `Row Labels` order by recon_code,dataset_code,rec_slno;
