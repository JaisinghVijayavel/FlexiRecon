...
DROP PROCEDURE IF EXISTS `pr_get_reportresultsetfieldtype` $$
CREATE PROCEDURE `pr_get_reportresultsetfieldtype`
(
  in_reporttemplate_code varchar(32),
  in_reporttemplateresultset_code varchar(32),
  out out_msg text,
  out out_result int
)
me:BEGIN
  ...
  */
  ...
  into
    ...
    v_report_name,
    v_report_exec_type
  from recon_mst_treporttemplateresultset as a
  inner join recon_mst_treporttemplate as b on a.reporttemplate_code = b.reporttemplate_code
    and b.active_status = 'Y'
    and b.delete_flag = 'N'
  inner join recon_mst_treport as c on a.src_report_code = c.report_code
    and c.delete_flag = 'N'
  ...
  if exists(select * from recon_mst_treporttemplatefield
    ...
    insert ignore into recon_tmp_tfield (field_name,field_alias_name,field_type,field_length,display_order,display_flag)
    select
      a.report_field,
      a.display_desc,
      if(
          v_report_exec_type = 'D',
          fn_get_dsfieldtype(v_report_code,a.report_field),
          fn_get_fieldtype(b.recon_code,a.report_field)
      ) as field_type,
  ...
  elseif v_report_exec_type = 'D' then
    ...
    insert ignore into recon_tmp_tfield (field_name,field_alias_name,field_type,field_length,display_order)
    select
      dataset_table_field,
      field_name,
      field_type,
    ...
    where dataset_code = v_report_code
    and delete_flag = 'N'
  ...
  elseif exists(select field_name from recon_mst_tsystemfield
    ...
    if v_recontype_code = 'W' or v_recontype_code = 'B' or v_recontype_code = 'I' then
      ...
      select
        field_name,
        fn_get_reconfieldname(v_recon_code,field_name),
        fn_get_fieldtype(v_recon_code,field_name) as field_type,
    ...
    select
      a.recon_field_name,
      fn_get_reconfieldname(v_recon_code,a.recon_field_name),
  ...
  elseif exists(select field_name from recon_mst_ttablestru
    ...
    select
      field_name,
      ifnull(display_desc,fn_get_reconfieldname(v_recon_code,field_name)),
      fn_get_fieldtype(v_recon_code,field_name) as field_type,
      @sno := @sno + 1
    ...
    insert ignore into recon_tmp_tfield (field_name,field_alias_name,field_type,field_length)
      SELECT
  ...
  if exists(select * from recon_mst_treporttemplatefield
    ...
    left join recon_mst_tsystemfield as b on b.report_field_name = a.report_field
      ...
      and b.delete_flag = 'N'
    where a.reporttemplate_code = in_reporttemplate_code
    and a.reporttemplate_code = in_reporttemplate_code
