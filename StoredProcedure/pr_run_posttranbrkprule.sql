...
me:BEGIN
  ...
  drop temporary table if exists recon_tmp_tvalue;
  ...
  CREATE temporary TABLE recon_tmp_tvalue(
  ...
  applyrule_block:begin
    ...
    applyrule_loop: loop
      ...
      set v_comparison_head_sql = concat(v_comparison_head_sql,' and excp_value > 0 and tran_gid = 0 ');
          ...
          basefilter_block:begin
            ...
            basefilter_loop: loop
              fetch basefilter_cursor into v_filter_applied_on,v_filter_field,
                                    v_filter_criteria,v_add_filter,v_ident_criteria,v_ident_value,
          ...
          create index idx_excp_mult_value on recon_tmp_tsource(excp_mult_value);
          ...
          create index idx_excp_mult_value on recon_tmp_tcomparison(excp_mult_value);
          create index idx_tran_date on recon_tmp_tcomparison(tran_date);
          create index idx_tran_gid on recon_tmp_tcomparison(tran_gid);
          create index idx_dataset_code on recon_tmp_tcomparison(recon_code,dataset_code,tran_acc_mode);
          ...
          rule_block:begin
            ...
            rule_loop: loop
              ...
              if (instr(v_extraction_criteria,'$FIELD$') > 0 or v_extraction_filter > 0)
                ...
                insert into recon_tmp_tsql(sql_query) values (v_sql);
          ...
          if v_group_flag = 'N' then
            ...
            truncate recon_tmp_tvalue;

            insert into recon_tmp_tvalue select distinct excp_value from recon_tmp_tsource;

            delete from recon_tmp_tcomparison where excp_value not in (select tran_value from recon_tmp_tvalue);

            truncate recon_tmp_tvalue;


            set v_match_sql = 'insert into recon_tmp_tmatch (tran_gid,tranbrkp_gid,matched_count,matched_value,scheduler_gid) ';
            ...
            set v_sql = '';
            set v_sql = concat(v_sql,'update recon_tmp_tmatch set matched_json = ');
            ...
            insert into recon_tmp_ttranbrkpgid
              select
                distinct JSON_UNQUOTE(JSON_EXTRACT(matched_json, CONCAT('$[', recon_tmp_tpseudorows.row, '].tranbrkp_gid'))) AS tranbrkp_gid
              FROM recon_tmp_tmatch
          ...
          insert into recon_tmp_ttrangid
            select tran_gid FROM recon_tmp_tmatch
            group by tran_gid
            having count(*) > 1;
          ...
          select
            ...
            JSON_UNQUOTE(JSON_EXTRACT(recon_tmp_tmatch.matched_json, CONCAT('$[', recon_tmp_tpseudorows.row, '].excp_value'))) AS excp_value
          FROM recon_tmp_tmatch
          JOIN recon_tmp_tpseudorows
          ...
          insert into recon_tmp_ttranbrkpgid
            select tranbrkp_gid FROM recon_tmp_tmatchdtl
