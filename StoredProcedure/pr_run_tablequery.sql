...
me:BEGIN
  ...
  */
  ...
  declare v_display_format text default '';
  ...
  if exists(select * from recon_mst_treporttemplatefield
    ...
    if v_report_exec_type = 'D' then
      insert into recon_tmp_tfield
      (
        field_name,field_alias_name,field_type,field_length,display_order,display_format
      )
      select
        a.report_field,
        a.display_desc,
        c.field_type as field_type,
        c.field_length as field_length,
        a.display_order,
        a.display_format
      from recon_mst_treporttemplatefield as a
      left join recon_mst_tdatasetfield as c on c.dataset_code = v_report_code
  ...
  elseif v_report_exec_type = 'D' then
    ...
    insert into recon_tmp_tfield
    (
      field_name,field_alias_name,field_type,field_length,display_order
    )
    select
      dataset_table_field,
      field_name,
  ...
  elseif exists(select field_name from recon_mst_tsystemfield
    ...
    if v_recontype_code = 'W' or v_recontype_code = 'B' or v_recontype_code = 'I' then
      insert into recon_tmp_tfield
      (
        field_name,field_alias_name,field_type,display_order
      )
      select
        field_name,
  ...
  elseif exists(select field_name from recon_mst_ttablestru
    ...
    insert into recon_tmp_tfield
    (
      field_name,field_alias_name,field_type,field_length
    )
    SELECT
  ...
  field_block:begin
    ...
    declare field_cursor cursor for
      select
        a.field_name,a.field_alias_name,a.field_type,a.field_length,a.display_format
      from recon_tmp_tfield as a
    ...
    field_loop: loop
      fetch field_cursor into v_field_name,v_field_alias_name,v_field_type,v_field_length,v_display_format;
      if field_done = 1 then leave field_loop; end if;

      set v_field_alias_name = ifnull(v_field_alias_name,v_field_name);
      set v_display_format = ifnull(v_display_format,'');

      if v_display_format = '' then
        ...
        set v_field_name = fn_get_fieldtypeformat(in_recon_code,v_field_name,v_field_type,v_field_length);
        set v_field_name = replace(v_display_format,'$FIELD$',v_field_name);
        set v_field = concat(v_field_name,' as ',char(39),v_field_alias_name,char(39));
      end if;
