...
me:BEGIN
  ...
  while (i > 0) do
		...
		insert into recon_tmp_trule select v_rule_code,v_system_match_flag,v_hold_flag;
  ...
  (
    rule_code, rule_name, recon_code, rule_apply_on, rule_order,
    source_dataset_code, source_acc_mode, comparison_dataset_code, comparison_acc_mode,
    group_flag, group_method_flag, manytomany_match_flag,
    reversal_flag, system_match_flag, manual_match_flag, hold_flag,
    period_from, period_to, until_active_flag,
    recon_rule_version,
    active_status,
    insert_date,
    insert_by
