...
DROP PROCEDURE IF EXISTS `pr_get_dashboard` $$
CREATE PROCEDURE `pr_get_dashboard`(
...
me:BEGIN
  ...
  */
  ...
  inner join recon_trn_tscheduler as s on t.scheduler_gid = s.scheduler_gid
    and s.insert_date >= '",cast(in_period_from as nchar),"'
    and s.insert_date < date_add('",cast(in_period_to as nchar),"',interval 1 day)
    LOCK IN SHARE MODE) as a");
  ...
  inner join ",v_kodtl_table," as d on t.tran_gid = d.tran_gid and d.delete_flag = 'N'
  inner join ",v_ko_table," as k on d.ko_gid = k.ko_gid and k.delete_flag = 'N'
  where t.tran_date >= '",cast(in_period_from as nchar),"'
  ...
  select
    count(distinct tran_gid) into v_ko_count
  from recon_tmp_tkodtlgid;
  ...
  select
    count(distinct tran_gid) into v_ko_system_count
  from recon_tmp_tkodtlgid
  where tran_date >= in_period_from
  and tran_date <= in_period_to
  ...
  select
    count(distinct tran_gid) into v_ko_zeroexcp_count
  from recon_tmp_ttrangid
  where excp_value = 0;

  select
    count(distinct tran_gid) into v_ko_partialexcp_count
  from recon_tmp_ttrangid
  where excp_value <> 0
