...
DROP PROCEDURE IF EXISTS `pr_run_automatchwihbrkp` $$
CREATE PROCEDURE `pr_run_automatchwihbrkp`(
      ...
      set v_comparison_headbrkp_sql = concat(v_comparison_headbrkp_sql,' and excp_value > 0 and tran_gid > 0 ');
          ...
          basefilter_block:begin
            ...
            basefilter_loop: loop
              ...
              set v_basefilter_condition = fn_get_basefilterformat(v_filter_field,v_filter_criteria,v_add_filter,v_ident_criteria,v_ident_value);
          ...
          set v_source_condition = '';
          set v_comparison_condition = '';
          ...
          rule_block:begin
            ...
            rule_loop: loop
              ...
              if v_source_field_org_type = 'TEXT' then
                set v_source_condition = concat(v_source_condition,'and ',v_source_field ,' <> '''' ');
              else
                set v_source_condition = concat(v_source_condition,'and ',v_source_field ,' is not null ');
              end if;

              if v_comparison_field_org_type = 'TEXT' then
                set v_comparison_condition = concat(v_comparison_condition,'and ',v_comparison_field ,' <> '''' ');
              else
                set v_comparison_condition = concat(v_comparison_condition,'and ',v_comparison_field ,' is not null ');
              end if;
              ...
              set v_build_condition = fn_get_comparisoncondition(in_recon_code,v_source_field_format,v_comparison_field,v_comparison_criteria,v_comparison_filter);

              set v_rule_condition = concat(v_rule_condition,'and ',v_build_condition,' ');

              if v_source_field_org_type = 'TEXT' then
                set v_rule_condition = concat(v_rule_condition,'and ',v_source_field ,' <> '''' ');
              else
                set v_rule_condition = concat(v_rule_condition,'and ',v_source_field ,' is not null ');
              end if;

              if v_comparison_field_org_type = 'TEXT' then
                set v_rule_condition = concat(v_rule_condition,'and ',v_comparison_field ,' <> '''' ');
              else
                set v_rule_condition = concat(v_rule_condition,'and ',v_comparison_field ,' is not null ');
              end if;
          ...
          if v_group_flag = 'Y' then
            ...
            if v_recon_value_flag = 'Y' then
              if v_recon_automatch_partial <> 'Y' then
                ...
                set v_match_sql = concat(v_match_sql,'"ko_value":', char(39),',cast(round(abs(sum(b.excp_value*b.tran_mult)),2) as nchar),',char(39));
              end if;
            else
              set v_match_sql = concat(v_match_sql,'"ko_value":', char(39),',cast(0 as nchar),',char(39));
            end if;
            ...
            -- many to many
            if v_manytomany_match_flag = 'Y' and v_recon_automatch_partial <> 'Y' and v_recon_value_flag = 'Y' then
