...
DROP PROCEDURE IF EXISTS `pr_get_integrity` $$
CREATE PROCEDURE `pr_get_integrity`
(
...
me:begin
  ...
  if v_min_tran_date is null
    ...
    call pr_get_intergrity_current(in_recon_code,in_tran_date,@msg,@result);
  else
    call pr_get_intergrity_rollback(in_recon_code,in_tran_date,@msg,@result);
  end if;
