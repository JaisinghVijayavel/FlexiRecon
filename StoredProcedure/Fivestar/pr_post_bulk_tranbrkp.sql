...
DROP PROCEDURE IF EXISTS `pr_post_bulk_tranbrkp` $$
CREATE PROCEDURE `pr_post_bulk_tranbrkp`(
...
me:BEGIN
  ...
  declare v_posted_count int default 0;
  ...
  */
  ...
  if in_recon_code is null or in_recon_code = '' then
    set @msg = 'Please select the recon';
    set @result = 0;
  ...
  if exists(select job_gid from recon_trn_tjob
    ...
    set out_msg = concat('Supporting file posting is already running in the job id ', v_txt ,' ! ');
    set out_result = 0;
  ...
  if in_automatch_flag = 'Y' then
    call pr_ins_job(in_recon_code,'S',0,'Posting Supporting File','',in_user_code,in_ip_addr,'I','Initiated',@out_job_gid,@msg,@result);
  else
    call pr_ins_job(in_recon_code,'P',0,'Preview Supporting File','',in_user_code,in_ip_addr,'I','Initiated',@out_job_gid,@msg,@result);
  end if;
      ...
      if v_txt_recon_code <> '' then
        ...
        if v_recon_gid > 0 then
          rule_loop: loop
            ...
            call pr_post_tranbrkpbyrule(v_recon_code,v_rule_code,v_job_gid,in_period_from,in_period_to,in_automatch_flag,in_user_code,@msg,@result);
